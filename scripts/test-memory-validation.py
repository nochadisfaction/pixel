...
<body>Page not found</body>
    ...
    validation_results = [(memory, service._validate_memory_content(memory)) for memory in test_memories]

    # Print results
    result_messages = [f"{'✅ VALID' if is_valid else '❌ INVALID'}: {memory.content[:50]}..." 
...
def test_backup_saving(service, test_memories):
    ...
    backup_summaries = [f"  {i+1}: {memory['content'][:50]}..." for i, memory in enumerate(saved_data)]
