...
function logMemoryUsage() {
  ...
  console.log(`[MEMORY] Process Heap: ${formatBytes(memInfo.process.heapUsed)} / ${formatBytes(memInfo.process.heapTotal)}`);
  console.log(`[MEMORY] System: ${formatBytes(memInfo.system.used)} / ${formatBytes(memInfo.system.total)} (${systemUsedPercent.toFixed(1)}%)`);
  
  // Trigger garbage collection if heap usage is high
  if (heapUsedMB > 3072) { // 3GB threshold
    console.log(`[MEMORY] ⚠️ High memory usage detected (${heapUsedMB.toFixed(0)}MB), triggering GC...`);
    if (global.gc) {
      global.gc();
      const afterGC = getMemoryInfo();
      const afterGCMB = afterGC.process.heapUsed / 1024 / 1024;
      console.log(`[MEMORY] ✅ GC completed. Heap: ${afterGCMB.toFixed(0)}MB (reduced by ${(heapUsedMB - afterGCMB).toFixed(0)}MB)`);
    } else {
      console.log(`[MEMORY] ❌ GC not available, consider running with --expose-gc`);
    }
  }
  
  // Log warning if system memory is very high
  if (systemUsedPercent > 85) {
    console.log(`[MEMORY] 🚨 Critical system memory usage: ${systemUsedPercent.toFixed(1)}%`);
  } else if (systemUsedPercent > 75) {
    console.log(`[MEMORY] ⚠️ High system memory usage: ${systemUsedPercent.toFixed(1)}%`);
  }
