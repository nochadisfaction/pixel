...
class BackupContentValidator:
    ...
    def create_cleaned_backup(self, source_file: Path, target_file: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Original: {results['original_count']}, Cleaned: {results['cleaned_count']}, Removed: {results['removed_count']}")
...
class BackupValidationCLI:
    ...
    def create_parser(self):
        ...
        parser.add_argument('--backup-dir', type=Path, default=Path('/home/vivi/pixel/memory_backups'),
                           help='Directory containing backup files')
        parser.add_argument('--validate-file', type=Path, help='Validate a specific backup file')
        parser.add_argument('--clean-file', type=Path, help='Create cleaned version of a backup file')
        parser.add_argument('--output-dir', type=Path, default=Path('/home/vivi/pixel/memory_backups/cleaned'),
    ...
    def validate_single_file(self, file_path: Path) -> bool:
        ...
        if results['invalid_memory_details']:
            ...
            for detail in results['invalid_memory_details'][:5]:
                print(f"  Index {detail['index']}: {detail.get('errors', detail.get('error', 'Unknown error'))}")
    ...
    def scan_backup_directory(self, backup_dir: Path) -> bool:
        ...
        if invalid_files := [f for f in results['file_results'] if not f['is_valid']]:
            ...
            for file_result in invalid_files[:3]:
                ...
                print(f"    Total: {file_result['total_memories']}, Valid: {file_result['valid_memories']}, Invalid: {file_result['invalid_memories']}")
