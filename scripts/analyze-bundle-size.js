    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        items.push({
          ...
          size: size,
          type: 'directory',
          children: currentDepth < maxDepth - 1 ? analyzeDirectory(fullPath, maxDepth, currentDepth + 1) : []
        });
      } else {
        try {
...
function printAnalysis(items, prefix = '', maxItems = 20) {
  items.slice(0, maxItems).forEach((item, index) => {
    ...
    if (item.children && item.children.length > 0 && item.size > 10 * 1024 * 1024) { // 10MB threshold
