...
const PROBLEMATIC_DEPS = {
  ...
  '@iconify/json': { size: '~383MB', reason: 'Complete icon library - should use selective imports' },
  '@iconify-json/carbon': { size: '~5MB', reason: 'Large icon set - consider minimal subset' },
  
  // Next.js related (not needed for Astro)
  '@next/swc-linux-x64-gnu': { size: '~142MB', reason: 'Next.js compiler - not needed for Astro builds' },
  '@next/swc-linux-x64-musl': { size: '~142MB', reason: 'Next.js compiler - not needed for Astro builds' },
  'next': { size: '~121MB', reason: 'Next.js framework - conflicts with Astro' },
  
  // Heavy ML/Graphics libraries
  '@tensorflow/tfjs': { size: '~141MB', reason: 'Large ML library - consider Lambda layers' },
  'three': { size: '~50MB', reason: '3D graphics library - externalize for Lambda' },
  
  // Development tools that shouldn't be in production
  'playwright': { size: '~100MB', reason: 'E2E testing tool - should be devDependency only' },
  'workerd': { size: '~99MB', reason: 'Cloudflare Worker runtime - not needed in Lambda' },
  
  // Old/deprecated SDKs
  'aws-sdk': { size: '~94MB', reason: 'AWS SDK v2 - should use v3 (@aws-sdk/client-*) for smaller bundles' },
  
  // Diagram libraries
  'mermaid': { size: '~61MB', reason: 'Diagram library - consider server-side rendering or CDN' }
};
...
function checkPackageJson() {
  try {
    ...
    const productionLeakRisks = ['@iconify/json', '@tensorflow/tfjs', 'three', 'playwright'];
    for (const dep of productionLeakRisks) {
      if (devDependencies[dep]) {
        console.log(`   ⚠️  ${dep} in devDependencies - ensure it doesn't leak to production`);
      }
    }
    ...
    // Recommendations
    console.log('\n💡 Recommendations:');
    console.log('   1. Move @iconify/json to devDependencies and use selective imports');
    console.log('   2. Replace aws-sdk with specific @aws-sdk/client-* packages');
