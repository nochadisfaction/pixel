...
class Mem0Setup {
  ...
  async checkPythonSetup() {
    ...
    try {
      ...
      const { stdout, stderr } = await execAsync(
        'python -X utf8 scripts/test_mem0.py',
      )

      // Check if tests passed
      const testsPassed =
        stdout.includes('All tests passed!') || stdout.includes('test passed')
  ...
  checkEnvironmentSetup() {
    ...
    if (!existsSync(this.envPath)) {
      ...
      console.log(
        '   You can get an API key from: https://app.mem0.ai/api-keys',
      )
      return false
    }
...
`
  ...
  async runTest() {
    ...
    console.log(
      `Environment configured: ${results.envConfigured ? '✅' : '❌'}`,
    )
  ...
  async run() {
    ...
    switch (command) {
      ...
      default:
        ...
        console.log('\nRun with commands:')
        console.log(
          '  node scripts/setup-mem0.js test     - Run full test suite',
        )
        console.log(
          '  node scripts/setup-mem0.js check    - Check installation only',
        )
        console.log(
          '  node scripts/setup-mem0.js template - Generate env template',
        )
    }
  }
}
