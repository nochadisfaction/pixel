...
class PsychologyPipelineDemo:
    ...
    def _step1_parse_knowledge(self) -> Dict[str, Any]:
        ...
        print(f"   ✓ Created PDM-2 framework with {len(pdm2_framework.patterns)} patterns → {pdm2_output}")
    ...
    def _step3_convert_conversations(self) -> Dict[str, Any]:
        ...
        self.conversation_converter.export_conversations_to_json(conversations, conversations_output)
        ...
        print(f"   ✓ Generated {len(conversations)} therapeutic conversations → {conversations_output}")
    ...
    def _step4_generate_responses(self, scenarios: List) -> Dict[str, Any]:
        ...
        responses = self.response_generator.generate_response_dataset(scenarios, responses_per_scenario=2)
    ...
    def _step5_validate_accuracy(self, conversations: List) -> Dict[str, Any]:
        ...
        avg_score = sum(v.overall_score for v in validations) / len(validations) if validations else 0
        ...
        print(f"   📊 Approved for training: {approved_count}/{len(validations)} ({approved_count/len(validations)*100:.1f}%)")
    ...
    def _step6_balance_categories(self, conversations: List, validations: List) -> Dict[str, Any]:
        ...
        print(f"   ✓ Balanced {balancing_result.total_items} items across categories → {balancing_output}")
    ...
    def _step7_generate_report(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        final_report = {
            ...
            "content_generation": {
                ...
                "therapeutic_conversations": results["conversation_conversion"]["total_conversations"],
            ...
            "final_dataset": {
                ...
                "target_achieved": results["category_balancing"]["total_balanced_items"] >= self.target_total * 0.8,
            ...
            "files_generated": [
                str(f.relative_to(self.output_dir)) for step_results in results.values()
                if isinstance(step_results, dict) and "output_file" in step_results
                for f in [step_results["output_file"]]
            ] + [
                str(f.relative_to(self.output_dir)) for f in results["knowledge_parsing"]["files_created"]
            ]
        }
