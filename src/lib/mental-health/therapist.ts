import type { MentalHealthAnalysis, TherapeuticResponse, HealthIndicator } from './types'
...
export class TherapeuticResponseGenerator {
  ...
  private selectApproach(analysis: MentalHealthAnalysis): 'supportive' | 'cognitive' | 'behavioral' | 'crisis' {
    if (analysis.riskLevel === 'critical' || analysis.requiresIntervention) {
      return 'crisis'
    }

    const hasThoughtPatterns = analysis.indicators.some(i => 
      i.type === 'depression' && i.evidence.some(e => 
        e.includes('worthless') || e.includes('hopeless') || e.includes('failure')
      )
    )

    const hasBehavioralIssues = analysis.indicators.some(i => 
      i.type === 'isolation' || (i.type === 'depression' && i.evidence.some(e => 
        e.includes('sleep') || e.includes('energy') || e.includes('tired')
      ))
    )
  ...
  private generateContent(analysis: MentalHealthAnalysis, approach: string): string {
  ...
  private selectTechniques(analysis: MentalHealthAnalysis, approach: string): string[] {
    const techniques: string[] = []
    
    switch (approach) {
      case 'crisis':
        techniques.push('Safety planning', 'Crisis intervention', 'Resource connection')
        break
      case 'cognitive':
        techniques.push('Thought challenging', 'Cognitive restructuring', 'Evidence examination')
        break
      case 'behavioral':
        techniques.push('Activity scheduling', 'Behavioral activation', 'Goal setting')
        break
      default:
    ...
    analysis.indicators.forEach(indicator => {
      switch (indicator.type) {
        ...
        case 'stress':
          ...
          break
        case 'isolation':
          techniques.push('Social connection planning', 'Support system building')
          break
      }
    })
  ...
  private generateFollowUp(analysis: MentalHealthAnalysis, approach: string): string[] {
    ...
    if (approach === 'crisis') {
      ...
      followUp.push('Can you commit to staying safe until we can get you more help?')
      return followUp
    }
    ...
    switch (primaryIndicator?.type) {
      case 'depression':
        ...
        followUp.push('Have you been able to talk to anyone else about how you\'re feeling?')
        break
      case 'anxiety':
        followUp.push('What physical sensations do you notice when you feel anxious?')
        followUp.push('Are there specific situations that tend to trigger your anxiety?')
        followUp.push('What helps you feel calmer when anxiety hits?')
        break
      case 'stress':
        followUp.push('What are the main sources of stress in your life right now?')
        followUp.push('How do you typically handle stress?')
        followUp.push('What would need to change for you to feel less overwhelmed?')
        break
      case 'isolation':
