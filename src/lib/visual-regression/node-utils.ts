...
export const getVisualRegressionData = async (): Promise<VisualRegressionData> => {
  try {
    ...
    const scanDirectory = (dir: string) => {
      try {
        ...
        for (const item of items) {
          ...
          if (stats.isDirectory()) {
          ...
          } else if (item.endsWith('-actual.png') || item.endsWith('-expected.png')) {
            const baseName = item.replace(/-(?:actual|expected)\.png$/, '')
            const page = path.relative(testResultsDir, dir).replace(/[/\\]/g, '/') || 'root'
            ...
            if (hasExpected || hasActual) {
              const screenshot: Screenshot = {
                name: baseName,
                page,
                deviceType: baseName.includes('mobile') ? 'mobile' : 'desktop',
