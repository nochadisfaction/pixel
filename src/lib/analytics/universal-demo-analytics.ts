...
export class UniversalDemoAnalytics {
  ...
  private getOrSetABTestVariant(): string {
    let variant = sessionStorage.getItem(ANALYTICS_CONFIG.STORAGE_KEYS.AB_VARIANT)
  ...
  private setupSectionTracking(): void {
    ...
    document.querySelectorAll('section, .hero-section, [data-section]').forEach(section => {
      observer.observe(section)
    })
  }

  private setupCTATracking(): void {
    document.querySelectorAll('[data-event="cta_click"], button, a[href]').forEach(element => {
  ...
  private setupDemoInteractionTracking(): void {
    ...
    const demoContainers = document.querySelectorAll('.bg-slate-900\\/50, [data-demo], .demo-container')
...
export function initializeDemoAnalytics(pageName: DemoPageName): UniversalDemoAnalytics {
