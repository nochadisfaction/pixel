...
export const DEMO_PAGES_CONFIG: Record<string, DemoPageConfig> = {
  'clinical-vault-trainer': {
    ...
    keyMetrics: ['crisis_detection_demo', 'persistent_patient_demo', 'encrypted_analysis_demo']
  },
  ...
  'synthetic-training-generator': {
    ...
    conversionGoals: ['patient_generation_demo', 'cta_click', 'technical_demo_request'],
    keyMetrics: ['generation_speed_demo', 'symptom_encoding_demo', 'cultural_variation_demo']
  },
  ...
  'psychology-pipeline-processor': {
    ...
    keyMetrics: ['processing_speed_demo', 'category_balancing_demo', 'quality_assurance_demo']
  },
  ...
  'security-bias-detection-engine': {
    ...
    conversionGoals: ['bias_detection_demo', 'cta_click', 'security_consultation'],
    keyMetrics: ['real_time_detection_demo', 'encrypted_analysis_demo', 'intervention_trigger_demo']
  }
}
...
export function calculateStatisticalSignificance(
  ...
  const pooledP = (controlConversions + testConversions) / (controlSessions + testSessions)
  
  const se = Math.sqrt(pooledP * (1 - pooledP) * (1/controlSessions + 1/testSessions))
  const zScore = Math.abs(p2 - p1) / se
  
  // Convert z-score to p-value (two-tailed test)
  const pValue = 2 * (1 - normalCDF(Math.abs(zScore)))
  const confidenceLevel = (1 - pValue) * 100
  const isSignificant = confidenceLevel >= ANALYTICS_CONFIG.REQUIRED_CONFIDENCE_LEVEL
  ...
  } else if (controlSessions + testSessions < ANALYTICS_CONFIG.MIN_CONVERSIONS_FOR_SIGNIFICANCE) {
