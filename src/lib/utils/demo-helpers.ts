...
export function calculateBiasFactors(sessionData: SessionData): BiasFactors {
  ...
  const biasKeywords = {
    cultural: ['culture', 'traditional', 'your people', 'background', 'heritage'],
    ...
    linguistic: ['speak slowly', 'simpler words', 'let me explain', 'understand better']
  }
 ...
 */
export function generateHistoricalComparison(currentBiasScore: number): HistoricalComparison {
  ...
  return {
    ...
    comparisonToAverage,
    trendDirection: trendDirection === 'improving' ? 'Improving ↘️' : 
                   trendDirection === 'worsening' ? 'Worsening ↗️' : 'Stable →'
  }
}
 ...
 */
export function generateRecommendations(biasFactors: BiasFactors, demographics: Demographics): string[] {
  const recommendations: string[] = []

  // Overall bias recommendations
  if (biasFactors.overall > 0.7) {
    recommendations.push('🚨 Critical bias detected - implement immediate intervention and supervision')
    recommendations.push('📚 Mandatory bias awareness training required before continuing client work')
  } else if (biasFactors.overall > 0.5) {
    recommendations.push('⚠️ High bias detected - schedule additional supervision and bias training')
  }

  // Specific bias type recommendations
  if (biasFactors.racial > 0.4) {
    recommendations.push('🌍 Review content for racial bias patterns and implement cultural humility practices')
    recommendations.push('📖 Complete cultural competency training focused on anti-racism in therapy')
  }

  if (biasFactors.gender > 0.4) {
    recommendations.push('⚧ Analyze gender-based assumptions and practice gender-affirming language')
    recommendations.push('🏳️‍⚧️ Complete training on LGBTQ+ affirming therapeutic practices')
  }

  if (biasFactors.age > 0.4) {
    recommendations.push('👥 Address ageist assumptions and practice age-appropriate communication')
    recommendations.push('📚 Study lifespan development to understand age-related diversity')
  }

  if (biasFactors.cultural > 0.4) {
    recommendations.push('🌏 Enhance cultural competency and avoid cultural generalizations')
    recommendations.push('🤝 Engage in cultural self-awareness exercises and bias reflection')
  }

  if (biasFactors.linguistic > 0.4) {
    recommendations.push('🗣️ Avoid assumptions about language proficiency and communication ability')
    recommendations.push('🌐 Learn about multilingual therapy best practices')
  }

  // Demographic-specific recommendations
  if (demographics.primaryLanguage !== 'en') {
    recommendations.push('🔄 Consider professional interpreter services for complex therapeutic work')
  }

  if (demographics.gender === 'non-binary' || demographics.gender === 'prefer-not-to-say') {
    recommendations.push('🏳️‍⚧️ Ensure consistent use of chosen pronouns and gender-affirming language')
  }

  // General improvement recommendations
  if (biasFactors.overall > 0.3) {
    recommendations.push('🎯 Practice mindfulness and self-reflection before and during sessions')
    recommendations.push('📝 Implement bias interruption techniques in real-time')
    recommendations.push('👥 Seek consultation with colleagues from diverse backgrounds')
  }
 ...
 */
export function getPresetScenariosByCategory(category: string): PresetScenario[] {
 ...
 */
export function getPresetScenariosByRiskLevel(riskLevel: string): PresetScenario[] {
 ...
 */
export function determineAlertLevel(biasScore: number): 'low' | 'medium' | 'high' | 'critical' {
