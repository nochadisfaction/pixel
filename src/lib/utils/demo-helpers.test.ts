...
describe('Demo Helpers', () => {
  ...
  beforeEach(() => {
    mockSessionData = {
      ...
      content: 'Therapist: You people from your culture tend to be more emotional about these things.',
  ...
  describe('calculateBiasFactors', () => {
    ...
    it('should calculate lower bias for inclusive content', () => {
      const inclusiveSessionData = {
        ...mockSessionData,
        content: 'Therapist: How are you feeling today? Can you tell me more about your experience?',
        demographics: {
          age: '26-35',
          gender: 'male',
          ethnicity: 'white',
    ...
    it('should cap bias scores at reasonable maximums', () => {
      const extremeSessionData = {
        ...mockSessionData,
        content: 'your people culture traditional background heritage naturally instinct emotional at your age elderly community neighborhood statistics show speak slowly simpler words'
      }
  ...
  describe('generateCounterfactualScenarios', () => {
    it('should generate scenarios for high bias factors', () => {
      ...
      const scenarios = generateCounterfactualScenarios(mockSessionData, biasFactors)
      expect(scenarios.length).toBeGreaterThan(3)
      
      const scenarioChanges = scenarios.map(s => s.change)
      expect(scenarioChanges.some(change => change.includes('Age'))).toBe(true)
      expect(scenarioChanges.some(change => change.includes('Cultural'))).toBe(true)
      expect(scenarioChanges.some(change => change.includes('Language'))).toBe(true)
    })
    ...
    it('should include therapeutic approach counterfactual', () => {
      ...
      const scenarios = generateCounterfactualScenarios(mockSessionData, biasFactors)
      expect(scenarios.some(s => s.change.includes('Therapeutic Language'))).toBe(true)
    })
    ...
    it('should assign appropriate likelihood levels', () => {
      ...
      const scenarios = generateCounterfactualScenarios(mockSessionData, biasFactors)
      const highLikelihoodScenarios = scenarios.filter(s => s.likelihood === 'high')
  ...
  describe('generateHistoricalComparison', () => {
    it('should generate realistic historical data', () => {
      ...
      expect(['improving', 'stable', 'worsening']).toContain(comparison.sevenDayTrend)
    })
    ...
    it('should correctly determine trend direction', () => {
      ...
      expect(['improving', 'stable', 'worsening']).toContain(lowBiasComparison.sevenDayTrend)
      expect(['improving', 'stable', 'worsening']).toContain(highBiasComparison.sevenDayTrend)
    })
  })
  ...
  describe('generateRecommendations', () => {
    it('should generate critical recommendations for high bias', () => {
      ...
      const recommendations = generateRecommendations(highBiasFactors, mockSessionData.demographics)
      expect(recommendations.some(r => r.includes('Critical bias detected'))).toBe(true)
      expect(recommendations.some(r => r.includes('racial bias patterns'))).toBe(true)
    ...
    it('should generate fewer recommendations for low bias', () => {
      ...
      const recommendations = generateRecommendations(lowBiasFactors, mockSessionData.demographics)
    ...
    it('should include demographic-specific recommendations', () => {
      ...
      const recommendations = generateRecommendations(biasFactors, nonBinaryDemographics)
  ...
  describe('createExportData', () => {
    it('should create properly structured export data', () => {
      ...
      const counterfactualScenarios = generateCounterfactualScenarios(mockSessionData, {
        overall: 0.5,
        linguistic: 0.4,
        gender: 0.3,
        racial: 0.4,
      ...
      const exportData = createExportData(mockAnalysisResults, counterfactualScenarios, historicalComparison)
