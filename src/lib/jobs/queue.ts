 ...
 * 
 * TODO: This implementation requires Redis hash and sorted set operations
 * that are not currently available in the RedisService. This needs to be
 * updated to either use the available RedisService methods or extend
 * RedisService with the required Redis operations.
 * 
 * TEMPORARY: Using type assertions to suppress Redis incompatibility errors.
 */

import { redis as redisService } from '../../lib/services/redis'
import { createLogger } from '../../utils/logger'
import { v4 as uuidv4 } from 'uuid'
import type Redis from 'ioredis'

// Get the raw Redis client which has all Redis operations
...
const logger = createLogger({ context: 'JobQueueService' })
...
export class JobQueueService {
  ...
  async enqueue<T>(type: string, payload: T, options?: EnqueueOptions): Promise<Job<T>> {
    ...
    if (options?.delay) {
      ...
      logger.info('Job enqueued with delay', { jobId: job.id, type: job.type, delay: options.delay })
    } else {
      await redis.zadd(this.queueKey, score, jobString)
      logger.info('Job enqueued', { jobId: job.id, type: job.type, priority: score })
    }
  ...
  async updateJobStatus(jobId: string, status: JobStatus, updates?: Partial<Job>): Promise<void> {
    ...
    if (currentJobString) {
      job = { ...JSON.parse(currentJobString), ...updates, status, updatedAt: new Date().toISOString() }
      ...
      logger.warn('Attempted to update status for non-existent job without full details', { jobId, status, updates });
      return;
    }
  ...
  async getJobsByStatus(status: JobStatus): Promise<Job[]> {
    ...
    switch (status) {
      ...
      case JobStatus.CANCELLED: {
        ...
        jobStrings = allFailedJobs.filter(jobStr => JSON.parse(jobStr).status === JobStatus.CANCELLED);
        break;
      }
      default:
        return [];
  ...
  async getJobIdsByStatus(status: JobStatus): Promise<string[]> {
    ...
    switch (status) {
      ...
      case JobStatus.CANCELLED: {
        ...
        jobIds = cancelledJobs.filter(jobStr => JSON.parse(jobStr).status === JobStatus.CANCELLED).map(jobStr => JSON.parse(jobStr).id);
        break;
      }
      default:
        return [];
  ...
  async clearAllJobs(): Promise<void> {
    ...
    if (allJobStatusKeys.length > 0) {
      await redis.del(...allJobStatusKeys);
    }
  ...
  async getJobCounts(): Promise<Record<JobStatus, number>> {
    ...
    const allFailedJobs = Object.values(await redis.hgetall(this.failedKey));
    const cancelledCount = allFailedJobs.filter(jobStr => JSON.parse(jobStr).status === JobStatus.CANCELLED).length;
