...
export const ParticipantDemographicsSchema = z.object({
  ...
  primaryLanguage: z
    .string()
    .min(2, 'Primary language must be at least 2 characters'),
...
export const BiasDetectionConfigSchema = z.object({
  ...
  thresholds: z
    ...
    .refine(
      (data) =>
        data.warningLevel < data.highLevel &&
        data.highLevel < data.criticalLevel,
      {
        message:
  ...
  evaluationMetrics: z
    .array(z.string())
    .min(1, 'At least one evaluation metric is required'),
  metricsConfig: z.any(),
  alertConfig: z.any(),
  reportConfig: z.any(),
...
export function sanitizeTextContent(
  ...
  sanitized = sanitized.replace(
    /\b(?!Patient\b|Doctor\b|Therapist\b|Mr\b|Mrs\b|Ms\b|Dr\b)[A-Z][a-z]+\s+[A-Z][a-z]+\b/g,
    '[NAME]',
  )
...
export function groupSessionsByDemographics(
  ...
  for (const session of sessions) {
    const demographicGroups = extractDemographicGroups(
 ...
 */
export function calculateConfidenceScore(layerResults: {
  preprocessing: { biasScore: number }
  modelLevel: { biasScore: number }
  interactive: { biasScore: number }
  evaluation: { biasScore: number }
...
export function calculateFairnessMetrics(
  ...
  if (groups.length < 2) {
    throw new Error(
      'At least two demographic groups required for fairness metrics',
    )
  }
  ...
  const demographicParity =
    Math.max(...positiveRates) - Math.min(...positiveRates)
...
export function generateAnalysisSummary(results: BiasAnalysisResult[]): {
  ...
  const averageBiasScore =
    results.reduce((sum, result) => sum + result.overallBiasScore, 0) /
    totalSessions

  // Count alert levels
  const alertDistribution: Record<string, number> = {}
  for (const result of results) {
    alertDistribution[result.alertLevel] =
      (alertDistribution[result.alertLevel] || 0) + 1
  }
