...
export class BiasDetectionDatabaseService {
  ...
  private async getSummaryStats(cutoffTime: Date): Promise<BiasSummaryStats> {
    try {
      ...
      const averageBiasScore = totalSessions > 0
        ? sessionStats.reduce((sum, s) => sum + s.overall_bias_score, 0) / totalSessions
        : 0
      ...
      if (totalAlertsError) {
        throw new Error(`Failed to get total alerts: ${totalAlertsError.message}`)
      }

      // Calculate improvement rate (simplified - compare with previous period)
      const previousCutoff = new Date(cutoffTime.getTime() - (Date.now() - cutoffTime.getTime()))
      ...
      if (previousError) {
        logger.warn('Failed to get previous period stats for improvement rate', {
          error: previousError.message,
        })
      }

      const previousAverage = previousStats && previousStats.length > 0
        ? previousStats.reduce((sum, s) => sum + s.overall_bias_score, 0) / previousStats.length
        : averageBiasScore
  ...
  private async getTrendData(timeRange: string): Promise<BiasTrendData[]> {
    try {
      ...
      for (let i = points - 1; i >= 0; i--) {
        ...
        const biasScore = sessionCount > 0
          ? analyses.reduce((sum, a) => sum + a.overall_bias_score, 0) / sessionCount
          : 0
        ...
        if (analyses && analyses.length > 0) {
          analyses.forEach(analysis => {
            const demographics = analysis.participant_demographics as ParticipantDemographics
    ...
    try {
      ...
      data?.forEach(row => {
        ...
        if (demo.primaryLanguage) {
          demographics.language[demo.primaryLanguage] = (demographics.language[demo.primaryLanguage] || 0) + 1
        }
      })
  ...
  private generateRecommendations(
    ...
    if (summary.criticalIssues > 0) {
      recommendations.push({
        ...
        estimatedImpact: 'High - Prevents potential harm and compliance violations',
      })
    }
