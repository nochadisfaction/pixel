...
import type { BiasDetectionConfig, TherapeuticSession, BiasAnalysisResult } from '../types';
...
describe('BiasDetectionEngine Integration Tests', () => {
    ...
    sampleSessions = [
      // Low bias scenario - experienced therapist with anxiety management
      {
        ...
        trainingScenario: {
          ...
          duration: 45,
          objectives: ['assess_anxiety', 'provide_coping_strategies', 'build_rapport']
        },
        sessionData: {
          transcript: 'Thank you for coming in today. I can see that you might be feeling anxious. Can you tell me more about what brings you here?',
          responses: ['I have been having trouble sleeping and feel overwhelmed at work'],
          interventions: ['active_listening', 'validation', 'anxiety_assessment'],
          outcomes: ['client_engaged', 'rapport_established', 'initial_assessment_complete']
        },
      ...
      // Medium bias scenario - beginner therapist with depression case
      {
        ...
        trainingScenario: {
          ...
          difficulty: 'advanced',
          duration: 60,
          objectives: ['assess_depression', 'explore_treatment_options', 'safety_assessment']
        },
        sessionData: {
          transcript: 'So you mentioned feeling sad. Have you considered that it might be because of your situation at home?',
          responses: ['I guess maybe, but it feels deeper than that'],
          interventions: ['assumption_making', 'leading_questions', 'incomplete_assessment'],
          outcomes: ['client_defensive', 'missed_cues', 'potential_bias_detected']
        },
      ...
      // High bias scenario - cultural mismatch with substance abuse case
      {
        ...
        trainingScenario: {
          ...
          difficulty: 'advanced',
          duration: 50,
          objectives: ['assess_substance_use', 'motivational_interviewing', 'treatment_planning']
        },
        sessionData: {
          transcript: 'Based on your background, I assume you probably have limited resources for treatment. Let me suggest some basic options.',
          responses: ['Actually, I have good insurance and want comprehensive treatment'],
          interventions: ['socioeconomic_assumptions', 'limited_treatment_options', 'cultural_stereotyping'],
          outcomes: ['client_corrected_assumptions', 'bias_indicators_high', 'therapeutic_alliance_damaged']
        },
  ...
  describe('Multi-Session Analysis and Reporting', () => {
    it('should analyze multiple sessions and generate comprehensive report', async () => {
      ...
      const report = await engine.generateBiasReport(sampleSessions, timeRange, {
        format: 'json',
        includeRawData: true,
        includeTrends: true,
  ...
  describe('Real-Time Monitoring Integration', () => {
    it('should provide real-time monitoring data during analysis', async () => {
      ...
      expect(monitoringData.systemHealth).toMatch(/^(healthy|warning|degraded|critical)$/);
  ...
  describe('Error Handling and Recovery', () => {
    ...
    it('should maintain data consistency during error recovery', async () => {
      ...
      expect(updatedMetrics.summary.totalAnalyses).toBe(initialAnalysisCount + 1);
    });
  });
  ...
  describe('Audit and Compliance Integration', () => {
    ...
    it('should provide explanation functionality for analysis results', async () => {
      ...
      const explanation = await engine.explainBiasDetection(result, undefined, true);
  ...
  describe('Metrics and Analytics Integration', () => {
    it('should provide comprehensive metrics after multiple analyses', async () => {
      ...
      expect(metrics.performance.systemHealth).toMatch(/^(healthy|warning|degraded|critical)$/);
