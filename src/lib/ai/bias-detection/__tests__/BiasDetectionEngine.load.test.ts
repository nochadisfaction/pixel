...
class LoadTestingUtils {
  ...
  static generateTestSessions(baseSession: TherapeuticSession, count: number): TherapeuticSession[] {
...
describe('Bias Detection Engine - Load Testing', () => {
  ...
  describe('Light Load Testing (5-10 concurrent sessions)', () => {
    it('should handle 5 concurrent sessions efficiently', async () => {
      const sessions = LoadTestingUtils.generateTestSessions(baselineAnxietyScenario, 5);
      
      const { metrics } = await LoadTestingUtils.runConcurrentSessions(biasEngine, sessions);
  ...
  describe('Moderate Load Testing (25 concurrent sessions)', () => {
    it('should maintain performance with 25 concurrent sessions', async () => {
      const sessions = LoadTestingUtils.generateTestSessions(baselineAnxietyScenario, 25);
  ...
  describe('Performance Benchmarking', () => {
    it('should meet minimum performance requirements', async () => {
      ...
      const multiSession = LoadTestingUtils.generateTestSessions(baselineAnxietyScenario, 5);

      // Measure single session performance
      const { metrics: singleMetrics } = await LoadTestingUtils.runConcurrentSessions(biasEngine, singleSession);
      
      // Measure concurrent session performance
      const { metrics: multiMetrics } = await LoadTestingUtils.runConcurrentSessions(biasEngine, multiSession);
      ...
      const performanceDegradation = multiMetrics.averageResponseTime / singleMetrics.averageResponseTime;
    ...
    it('should handle resource contention gracefully', async () => {
      const sessions = LoadTestingUtils.generateTestSessions(ageBiasYoungPatient, 15);
      
      const { metrics } = await LoadTestingUtils.runConcurrentSessions(biasEngine, sessions);
      LoadTestingUtils.logMetrics(metrics, 'Resource Contention Test (15 sessions)');
