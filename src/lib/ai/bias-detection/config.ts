...
export function validateConfig(config: Partial<BiasDetectionConfig>): void {
  ...
  if (config.thresholds) {
    ...
    if (warning >= high || high >= critical) {
      errors.push('Thresholds must be in ascending order: warningLevel < highLevel < criticalLevel');
    }
  }
  ...
  if (config.layerWeights) {
    ...
    Object.entries(weights).forEach(([layer, weight]) => {
      if (weight !== undefined && (weight < 0 || weight > 1)) {
        errors.push(`layerWeights.${layer} must be between 0.0 and 1.0, got ${weight}`);
      }
    });
    
    // Check total sum
    const total = (weights.preprocessing ?? 0) + 
                  (weights.modelLevel ?? 0) + 
                  (weights.interactive ?? 0) + 
                  (weights.evaluation ?? 0);
    
    if (Math.abs(total - 1.0) > 0.001) {
      errors.push(`Layer weights must sum to 1.0, current sum is ${total.toFixed(3)}`);
    }
  }
  ...
  if (config.pythonServiceUrl) {
    try {
      ...
      errors.push(`pythonServiceUrl must be a valid URL, got: ${config.pythonServiceUrl}`);
    }
  }

  if (config.pythonServiceTimeout !== undefined) {
    if (config.pythonServiceTimeout < 1000 || config.pythonServiceTimeout > 300000) {
      errors.push(`pythonServiceTimeout must be between 1000ms and 300000ms, got ${config.pythonServiceTimeout}ms`);
    }
  }

  // Validate evaluation metrics
  if (config.evaluationMetrics) {
    const validMetrics = ['demographic_parity', 'equalized_odds', 'calibration', 'individual_fairness'];
    const invalidMetrics = config.evaluationMetrics.filter(metric => !validMetrics.includes(metric));
    if (invalidMetrics.length > 0) {
      errors.push(`Invalid evaluation metrics: ${invalidMetrics.join(', ')}. Valid options: ${validMetrics.join(', ')}`);
    }
  }

  // Validate HIPAA compliance settings
  if (config.hipaaCompliant !== undefined && typeof config.hipaaCompliant !== 'boolean') {
    errors.push('hipaaCompliant must be a boolean value');
  }

  if (config.auditLogging !== undefined && typeof config.auditLogging !== 'boolean') {
 ...
 */
export function mergeWithDefaults(userConfig?: Partial<BiasDetectionConfig>): BiasDetectionConfig {
  if (!userConfig) {
...
export function loadConfigFromEnv(): Partial<BiasDetectionConfig> {
  ...
  if (process.env.BIAS_WEIGHT_PREPROCESSING) {
    layerWeights.preprocessing = parseFloat(process.env.BIAS_WEIGHT_PREPROCESSING);
  }
  ...
  if (process.env.BIAS_EVALUATION_METRICS) {
    envConfig.evaluationMetrics = process.env.BIAS_EVALUATION_METRICS.split(',').map(m => m.trim());
  }
  ...
  if (process.env.BIAS_ALERT_EMAIL_RECIPIENTS) {
    alertConfig.emailRecipients = process.env.BIAS_ALERT_EMAIL_RECIPIENTS.split(',').map(e => e.trim());
    alertConfig.enableEmailNotifications = true;
  }
  if (process.env.BIAS_ALERT_COOLDOWN_MINUTES) {
    alertConfig.alertCooldownMinutes = parseInt(process.env.BIAS_ALERT_COOLDOWN_MINUTES);
  }
  ...
  if (process.env.BIAS_METRICS_RETENTION_DAYS) {
    metricsConfig.metricsRetentionDays = parseInt(process.env.BIAS_METRICS_RETENTION_DAYS);
  }
  if (process.env.BIAS_DASHBOARD_REFRESH_RATE) {
    metricsConfig.dashboardRefreshRate = parseInt(process.env.BIAS_DASHBOARD_REFRESH_RATE);
  }
 ...
 */
export function getEnvironmentConfigSummary(): { loaded: string[]; available: string[] } {
  ...
  const loaded = availableEnvVars.filter(varName => process.env[varName] !== undefined);
 ...
 */
export function createConfigWithEnvOverrides(userConfig?: Partial<BiasDetectionConfig>): BiasDetectionConfig {
  const envConfig = loadConfigFromEnv();
  
  // Merge in order of precedence: defaults < env < user
  const combinedConfig = deepMergeConfigs(DEFAULT_CONFIG, envConfig, userConfig || {});
...
export function updateConfiguration(
  ...
  const updatedConfig = deepMergeConfigs(currentConfig, updates) as BiasDetectionConfig;
