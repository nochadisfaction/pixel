...
describe('PatientProfileService', () => {
  ...
  beforeEach(() => {
    ...
    sampleProfile = {
      ...
      cognitiveModel: {
        ...
        demographicInfo: { age: 30, gender: 'NB', occupation: 'Artist', familyStatus: 'Single', culturalFactors: [] },
        presentingIssues: ['existential dread'],
        diagnosisInfo: { primaryDiagnosis: 'Adjustment Disorder', secondaryDiagnoses: [], durationOfSymptoms: '2 months', severity: 'mild' },
        coreBeliefs: [],
        distortionPatterns: [],
        behavioralPatterns: [],
        emotionalPatterns: [],
  ...
  describe('saveProfile and getProfileById', () => {
    it('should save and retrieve a profile including new therapeutic alliance metrics', async () => {
      ...
      const retrievedProfile = await patientProfileService.getProfileById(sampleProfile.id);

      expect(mockKvStore.set).toHaveBeenCalledWith(
        `${profilePrefix}${sampleProfile.id}`,
        expect.objectContaining({ // ensure lastUpdatedAt is handled by saveProfile
            ...sampleProfile,
            lastUpdatedAt: expect.any(String)
        })
      );
      expect(retrievedProfile).toEqual(sampleProfile);
      expect(retrievedProfile?.cognitiveModel.therapeuticProgress.trustLevel).toBe(5);
      expect(retrievedProfile?.cognitiveModel.therapeuticProgress.rapportScore).toBe(6);
      expect(retrievedProfile?.cognitiveModel.therapeuticProgress.therapistPerception).toBe('neutral');
      expect(retrievedProfile?.cognitiveModel.therapeuticProgress.transferenceState).toBe('none');
    });
  ...
  describe('addMessageToPatientHistory', () => {
    it('should add a message and save the profile, preserving new alliance metrics', async () => {
        ...
        // Act
        const updatedProfile = await patientProfileService.addMessageToPatientHistory(
            sampleProfile.id,
            "Hello therapist",
            "patient"
        );
        ...
        // Assert
        expect(mockKvStore.get).toHaveBeenCalledWith(`${profilePrefix}${sampleProfile.id}`);
        expect(mockKvStore.set).toHaveBeenCalledTimes(1);

        const savedProfileArgument = mockKvStore.set.mock.calls[0][1] as PatientProfile;
        expect(savedProfileArgument.conversationHistory.length).toBe(1);
        expect(savedProfileArgument.conversationHistory[0].content).toBe("Hello therapist");
        // Crucially, check if the alliance metric is preserved
        expect(savedProfileArgument.cognitiveModel.therapeuticProgress.trustLevel).toBe(initialTrust);

        expect(updatedProfile?.cognitiveModel.therapeuticProgress.trustLevel).toBe(initialTrust);
  ...
  describe('getAvailableProfiles', () => {
    it('should retrieve identifiers including names from cognitive models', async () => {
      ...
      mockKvStore.keys.mockResolvedValue([`${profilePrefix}test-profile-1`, `${profilePrefix}test-profile-2`]);
      mockKvStore.get
        .mockResolvedValueOnce(sampleProfile)
        .mockResolvedValueOnce(profile2);

      const availableProfiles = await patientProfileService.getAvailableProfiles();
