...
import type { CognitiveModel, CoreBelief, TherapeuticProgress, SkillAcquired, TherapeuticInsight } from '../../types/CognitiveModel';
...
describe('TherapeuticProgressService', () => {
  ...
  beforeEach(() => {
    ...
    const initialCoreBeliefs: CoreBelief[] = [
      { belief: 'I am worthless', strength: 0.8, evidence: [], formationContext: '', relatedDomains: [] },
      { belief: 'The world is dangerous', strength: 0.7, evidence: [], formationContext: '', relatedDomains: [] },
    ];
    ...
    const initialCognitiveModel: CognitiveModel = {
      id: 'patient123',
      name: 'Test Patient',
      demographicInfo: { age: 30, gender: 'female', occupation: 'engineer', familyStatus: 'single', culturalFactors: [] },
      presentingIssues: ['Anxiety', 'Low self-esteem'],
      diagnosisInfo: { primaryDiagnosis: 'GAD', secondaryDiagnoses: [], durationOfSymptoms: '2 years', severity: 'moderate' },
      ...
      conversationalStyle: { verbosity: 0.5, emotionalExpressiveness: 0.5, insightLevel: 0.5, preferredCommunicationModes:[]},
  ...
  describe('addInsight', () => {
    it('should add a new insight to the profile', () => {
      const insightText = 'Realizing that my fear of failure is linked to childhood experiences.';
      const relatedBelief = 'I am worthless';
      const originalInsightCount = mockPatientProfile.cognitiveModel.therapeuticProgress.insights.length;

      const updatedProfile = service.addInsight(mockPatientProfile, insightText, relatedBelief);
      ...
      expect(updatedProfile.lastUpdatedAt).not.toBe(mockPatientProfile.lastUpdatedAt);
    });
    ...
    it('should throw an error if insight text is empty', () => {
      expect(() => service.addInsight(mockPatientProfile, '')).toThrow('Insight text cannot be empty.');
    });

    it('should throw an error if profile is invalid', () => {
        expect(() => service.addInsight(null as any, 'insight')).toThrow('Invalid patient profile provided.');
        const incompleteProfile = { ...mockPatientProfile, cognitiveModel: { ...mockPatientProfile.cognitiveModel, therapeuticProgress: undefined as any }};
        expect(() => service.addInsight(incompleteProfile, 'insight')).toThrow('Invalid patient profile provided.');
    });
  });
  ...
  describe('updateBeliefStrength', () => {
    ...
    it('should update the strength of an existing core belief', async () => {
      const originalBelief = mockPatientProfile.cognitiveModel.coreBeliefs.find(b => b.belief === targetBelief);
      ...
      const updatedProfile = service.updateBeliefStrength(mockPatientProfile, targetBelief, changeFactor);
      const updatedBelief = updatedProfile.cognitiveModel.coreBeliefs.find(b => b.belief === targetBelief);

      expect(updatedBelief).toBeDefined();
      expect(updatedBelief!.strength).toBeCloseTo(originalStrength + changeFactor);
      expect(updatedProfile.lastUpdatedAt).not.toBe(originalTimestamp);
    });

    it('should clamp belief strength between 0 and 1', () => {
      let updatedProfile = service.updateBeliefStrength(mockPatientProfile, targetBelief, 0.5); // 0.8 + 0.5 = 1.3 -> 1.0
      let updatedBelief = updatedProfile.cognitiveModel.coreBeliefs.find(b => b.belief === targetBelief);
      expect(updatedBelief!.strength).toBe(1);

      updatedProfile = service.updateBeliefStrength(mockPatientProfile, targetBelief, -1.0); // 0.8 - 1.0 = -0.2 -> 0.0
      updatedBelief = updatedProfile.cognitiveModel.coreBeliefs.find(b => b.belief === targetBelief);
      expect(updatedBelief!.strength).toBe(0);
    });

    it('should throw an error if the belief to update is not found', () => {
      expect(() => service.updateBeliefStrength(mockPatientProfile, nonExistentBelief, 0.1))
        .toThrow(`Belief "${nonExistentBelief}" not found.`);
    });

    it('should throw an error if belief text is empty', () => {
        expect(() => service.updateBeliefStrength(mockPatientProfile, '', 0.1)).toThrow('Belief text cannot be empty.');
    });

    it('should throw an error if profile or coreBeliefs are invalid', () => {
        expect(() => service.updateBeliefStrength(null as any, targetBelief, 0.1)).toThrow('Invalid patient profile or core beliefs missing.');
        const incompleteProfile = { ...mockPatientProfile, cognitiveModel: { ...mockPatientProfile.cognitiveModel, coreBeliefs: undefined as any }};
        expect(() => service.updateBeliefStrength(incompleteProfile, targetBelief, 0.1)).toThrow('Invalid patient profile or core beliefs missing.');
    });
  });
  ...
  describe('acquireSkill', () => {
    ...
    it('should add a new skill to the profile if it does not exist', () => {
      const originalSkillCount = mockPatientProfile.cognitiveModel.therapeuticProgress.skillsAcquired.length;
      const updatedProfile = service.acquireSkill(mockPatientProfile, skillName, initialProficiency);
      const newSkills = updatedProfile.cognitiveModel.therapeuticProgress.skillsAcquired;
    ...
    it('should update an existing skill if acquireSkill is called again for it', async () => {
      // First acquisition
      let profileWithSkill = service.acquireSkill(mockPatientProfile, skillName, 0.2);
      const firstSkill = profileWithSkill.cognitiveModel.therapeuticProgress.skillsAcquired.find(s => s.skillName === skillName);
      ...
      profileWithSkill = service.acquireSkill(profileWithSkill, skillName, newProficiency, ['during meetings']);

      const updatedSkills = profileWithSkill.cognitiveModel.therapeuticProgress.skillsAcquired;
    ...
    it('should default proficiency to 0.1 if not provided', () => {
        const updatedProfile = service.acquireSkill(mockPatientProfile, "Another Skill");
        const addedSkill = updatedProfile.cognitiveModel.therapeuticProgress.skillsAcquired.find(s => s.skillName === "Another Skill");
        expect(addedSkill!.proficiency).toBe(0.1);
    });

    it('should clamp initial proficiency between 0 and 1', () => {
      let updatedProfile = service.acquireSkill(mockPatientProfile, "Skill A", 1.5);
      let skill = updatedProfile.cognitiveModel.therapeuticProgress.skillsAcquired.find(s => s.skillName === "Skill A");
      expect(skill!.proficiency).toBe(1);

      updatedProfile = service.acquireSkill(mockPatientProfile, "Skill B", -0.5);
      skill = updatedProfile.cognitiveModel.therapeuticProgress.skillsAcquired.find(s => s.skillName === "Skill B");
      expect(skill!.proficiency).toBe(0);
    });

    it('should throw error for empty skill name', () => { // Renamed and modified
        expect(() => service.acquireSkill(mockPatientProfile, '')).toThrow('Skill name cannot be empty.');
  ...
  describe('updateSkillProficiency', () => {
    ...
    beforeEach(() => {
      ...
      mockPatientProfile = service.acquireSkill(mockPatientProfile, skillName, 0.2);
    });

    it('should update the proficiency of an existing skill', async () => {
      const newProficiency = 0.6;
      const originalSkill = mockPatientProfile.cognitiveModel.therapeuticProgress.skillsAcquired.find(s => s.skillName === skillName);
      const originalDate = originalSkill!.dateAchieved;

      // Wait a moment
      await new Promise(resolve => setTimeout(resolve, 10));

      const updatedProfile = service.updateSkillProficiency(mockPatientProfile, skillName, newProficiency);
      const updatedSkill = updatedProfile.cognitiveModel.therapeuticProgress.skillsAcquired.find(s => s.skillName === skillName);
    ...
    it('should clamp new proficiency between 0 and 1', () => {
      let updatedProfile = service.updateSkillProficiency(mockPatientProfile, skillName, 1.5);
      let skill = updatedProfile.cognitiveModel.therapeuticProgress.skillsAcquired.find(s => s.skillName === skillName);
      expect(skill!.proficiency).toBe(1);

      updatedProfile = service.updateSkillProficiency(mockPatientProfile, skillName, -0.5);
      skill = updatedProfile.cognitiveModel.therapeuticProgress.skillsAcquired.find(s => s.skillName === skillName);
      expect(skill!.proficiency).toBe(0);
    });

    it('should throw an error if the skill to update is not found', () => {
      const nonExistentSkill = 'Time Travel';
      expect(() => service.updateSkillProficiency(mockPatientProfile, nonExistentSkill, 0.5))
        .toThrow(`Skill "${nonExistentSkill}" not found. Cannot update proficiency.`);
    });
