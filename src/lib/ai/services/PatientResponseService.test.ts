...
describe('PatientResponseService', () => {
  ...
  describe('createResponseContext', () => {
    it('should create a response context successfully', async () => {
      ...
      const context = await responseService.createResponseContext('ctx1', baseStyleConfig, ['anxiety'], 2);
    ...
    it('should return null if profile not found for response context', async () => {
      ...
      const context = await responseService.createResponseContext('nonexistent', baseStyleConfig);
    ...
    it('should derive session number if not provided in createResponseContext', async () => {
        ...
        const context = await responseService.createResponseContext('ctx2', baseStyleConfig);
        expect(context?.sessionNumber).toBe(2);

        const profileDataNoLog = createTestPatientProfile('ctx3', 'Session Deriver No Log');
        profileDataNoLog.conversationHistory.push({role: 'patient', content: 'hi', timestamp: ''}); // Ensure logLength > 0
        ...
        const context3 = await responseService.createResponseContext('ctx3', baseStyleConfig);
  ...
  describe('generateConsistentResponse', () => {
    it('should return candidate response if it is consistent', async () => {
      ...
      const context: ResponseContext = { profile, styleConfig: baseStyleConfig, sessionNumber: 1 };
    ...
    it('should return a therapeutic response if candidate is inconsistent', async () => {
     ...
     it('should handle missing profile in context gracefully for generateConsistentResponse', async () => {
        ...
        const context = { styleConfig: baseStyleConfig, sessionNumber: 1 } as ResponseContext;
  ...
  describe('generatePatientPrompt', () => {
    const patientProfile = createTestPatientProfile('promptUser1', 'Prompt User', [
      { belief: 'I must be perfect', strength: 0.9, evidence: [], formationContext: '', relatedDomains: [] },
      { belief: 'The world is dangerous', strength: 0.7, evidence: [], formationContext: '', relatedDomains: [] },
    ]);
    patientProfile.conversationHistory = [
        { role: 'therapist', content: 'How are you feeling today?', timestamp: new Date().toISOString() },
        { role: 'patient', content: 'A bit anxious.', timestamp: new Date().toISOString() },
    ];
    ...
    it('should include basic patient info and style in prompt', () => {
      ...
      expect(prompt).toContain('Your core beliefs include: I must be perfect, The world is dangerous.');
      expect(prompt).toContain(`Your openness level is ${baseStyleConfig.openness}/10.`);
      expect(prompt).toContain(`Your coherence level is ${baseStyleConfig.coherence}/10.`);
      expect(prompt).toContain(`Your defense level is ${baseStyleConfig.defenseLevel}/10.`);
      expect(prompt).toContain(`Your disclosure style is ${baseStyleConfig.disclosureStyle}.`);
      expect(prompt).toContain(`You respond to challenges in a ${baseStyleConfig.challengeResponses} way.`);
      expect(prompt).toContain('The current therapeutic focus areas are: managing anxiety.');
    ...
    it('should correctly include new emotional authenticity parameters in prompt', () => {
      ...
      const context: ResponseContext = { profile: patientProfile, styleConfig: specificStyle, sessionNumber: 1 };
      const prompt = responseService.generatePatientPrompt(context);

      expect(prompt).toContain('Your emotional expression should be subtle.');
      expect(prompt).toContain('The intensity of your expressed emotion should be around 3/10.');
      expect(prompt).toContain('Focus on conveying sadness.');
      expect(prompt).toContain('Include textual descriptions of non-verbal cues (e.g., *sighs*, *looks away*, *nods slowly*) in a style that is descriptive.');
    });
    ...
    it('should correctly include new resistance and defensive mechanism parameters in prompt', () => {
      ...
      expect(prompt).toContain('Your resistance to therapeutic suggestions is 8/10.');
      expect(prompt).toContain('You are currently employing deflection as a defensive mechanism.');
      expect(prompt).toContain('Try to subtly change the subject or avoid direct answers if the topic feels uncomfortable.');
    });
    ...
    it('should include specific instruction for intellectualization defense', () => {
        ...
        const context: ResponseContext = { profile: patientProfile, styleConfig: specificStyle, sessionNumber: 1 };
        const prompt = responseService.generatePatientPrompt(context);
        expect(prompt).toContain('Focus on abstract concepts and avoid expressing direct feelings.');
    });
    ...
    it('should include instruction for emotional transitions', () => {
      const context: ResponseContext = { profile: patientProfile, styleConfig: baseStyleConfig, sessionNumber: 1 };
      const prompt = responseService.generatePatientPrompt(context);
      expect(prompt).toContain("Consider your previous emotional state and the therapist's last statement when forming your response, allowing for natural emotional shifts or intensifications.");
      expect(prompt).toContain("Maintain consistency with your established beliefs and history, but allow for emotional evolution within the conversation.");
    });
    ...
    it('should handle "none" for nonVerbalIndicatorStyle and activeDefensiveMechanism', () => {
      ...
      expect(prompt).not.toContain('Include textual descriptions of non-verbal cues');
