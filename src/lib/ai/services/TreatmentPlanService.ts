...
export class TreatmentPlanService {
  ...
  public generateTreatmentPlan(profile: PatientProfile): string {
    ...
    if (coreBeliefs.some(b => b.belief.toLowerCase().includes("worthless") || b.belief.toLowerCase().includes("failure"))) {
    ...
    if (therapeuticProgress && therapeuticProgress.skillsAcquired && therapeuticProgress.skillsAcquired.length > 0) {
