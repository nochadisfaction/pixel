...
export interface TreatmentTechnique {
  ...
  name: string
  description: string
  category: 'cognitive' | 'behavioral' | 'somatic' | 'mindfulness' | 'exposure' | 'interpersonal'
...
export interface SupportingPattern {
  type: 'symptom' | 'behavior' | 'cognition' | 'emotion' | 'risk_factor' | 'protective_factor'
  category: string
  description: string
...
export interface RecommendationOptions {
  ...
  techniqueFilter?: ('cognitive' | 'behavioral' | 'somatic' | 'mindfulness' | 'exposure' | 'interpersonal')[]
...
export class RecommendationService {
  ...
  async getRecommendations(
    ...
    try {
      ...
      const filteredRecommendations = this.applyFilters(finalRecommendations, options)
  ...
  async getRecommendationsFromAnalysis(
    ...
    try {
      ...
      if (analysis.isCrisis) {
        const crisisRecommendations = await this.generateCrisisRecommendations(analysis, context)
  ...
  async getCrisisRecommendations(
    ...
    logger.warn('Generating crisis recommendations', { clientId, crisisContext })
    ...
    recommendations.push({
      ...
      expectedOutcomes: ['Immediate safety', 'Risk reduction', 'Professional support connection'],
      riskConsiderations: ['Requires immediate professional oversight', 'May need emergency services'],
      progressMetrics: {
        measurementTools: ['Safety plan completion', 'Risk assessment scores'],
        checkpointIntervals: ['Immediate', '2 hours', '24 hours'],
        successCriteria: ['Safety plan in place', 'Professional support engaged', 'Risk level reduced']
      },
      metadata: {
    ...
    if (crisisContext.immediateSupport) {
      recommendations.push({
        ...
        expectedOutcomes: ['Immediate professional support', 'Crisis de-escalation', 'Safety planning'],
        riskConsiderations: ['Requires qualified professional', 'May involve emergency services'],
        progressMetrics: {
          measurementTools: ['Professional contact established', 'Crisis resolution status'],
          checkpointIntervals: ['Immediate', '1 hour', '4 hours'],
          successCriteria: ['Professional support engaged', 'Crisis stabilized', 'Follow-up planned']
        },
        metadata: {
          generatedAt: now.toISOString(),
          basedOnSessions: [],
   ...
   */
  private validateRecommendationRequest(clientId: string, context: RecommendationContext): void {
   ...
   */
  private async analyzeClientState(context: RecommendationContext): Promise<ClientState> {
    // Combine recent analyses to understand current state
    const latestAnalysis = context.mentalHealthAnalyses?.[0]
    const riskIndicators = context.recentSessions.length > 0 
  ...
  private async personalizeRecommendations(
    ...
    for (let i = 0; i < baseRecommendations.length; i++) {
      ...
      const recommendation: TreatmentRecommendation = {
        ...
        validUntil: new Date(Date.now() + this.DEFAULT_VALID_DURATION).toISOString(),
  ...
  private async addAdaptations(
    ...
    return recommendations.map(rec => ({
      ...rec,
      adaptations: {
        culturalFactors: this.identifyCulturalAdaptations(context.clientProfile),
   ...
   */
  private getFallbackRecommendations(_context: Partial<RecommendationContext>): TreatmentRecommendation[] {
    ...
    return [
      {
        ...
        validUntil: new Date(now.getTime() + this.DEFAULT_VALID_DURATION).toISOString(),
        timeframe: 'Within 1-2 weeks',
        rationale: 'Comprehensive assessment provides foundation for evidence-based treatment planning',
        expectedOutcomes: ['Clear treatment plan', 'Identified goals', 'Appropriate interventions'],
  ...
  private createMinimalProfile(clientId: string, partialProfile?: Partial<ClientProfile>): ClientProfile {
    return {
      id: clientId,
  ...
  private async createRecommendationFromIntervention(
    ...
    return {
      ...
      timeframe: this.mapUrgencyToTimeframe(intervention.urgency),
      rationale: intervention.rationale,
      expectedOutcomes: this.generateOutcomesForCategory(analysis.mentalHealthCategory),
      riskConsiderations: this.getRiskConsiderationsForCategory(analysis.mentalHealthCategory),
      progressMetrics: {
        measurementTools: this.getAssessmentToolsForCategory(analysis.mentalHealthCategory),
        checkpointIntervals: ['1 week', '2 weeks', '4 weeks'],
        successCriteria: ['Symptom reduction', 'Improved functioning', 'Goal achievement']
      },
      metadata: {
        generatedAt: now.toISOString(),
        basedOnSessions: [],
  ...
  private mapUrgencyToTimeframe(urgency: string): string {
    switch (urgency) {
      ...
      case 'urgent': return 'Within 1 week'
      case 'routine': return 'Within 2-4 weeks'
  ...
  private async extractRiskIndicators(sessions: TherapySession[]): Promise<string[]> {
  ...
  private identifyRiskConsiderations(
    ...
    if (context.clientProfile.currentStatus.riskLevel === 'high') {
      considerations.push('Requires careful monitoring due to elevated risk level')
    }
  ...
  private generateProgressMetrics(
    ...
    return {
      ...
      successCriteria: ['Symptom improvement', 'Functional gains', 'Goal achievement']
    }
  }
  ...
  private identifyIndividualAdaptations(context: RecommendationContext): string[] {
  ...
  private identifyContraindications(
    ...
    context.clientProfile.treatmentHistory.ineffectiveInterventions.forEach(intervention => {
  ...
  private determineCheckpointIntervals(recommendation: TreatmentRecommendation): string[] {
    switch (recommendation.priority) {
      case 'critical': return ['24 hours', '1 week', '2 weeks']
      case 'high': return ['1 week', '2 weeks', '4 weeks']
      case 'medium': return ['2 weeks', '4 weeks', '8 weeks']
      default: return ['4 weeks', '8 weeks', '12 weeks']
    }
  }
  ...
  private async selectTechniquesForIntervention(intervention: InterventionSuggestion): Promise<TreatmentTechnique[]> {
  ...
  private calculateInterventionEvidenceStrength(intervention: InterventionSuggestion): number {
    // Base evidence strength calculation
    switch (intervention.urgency) {
      case 'immediate': return 0.95
      case 'urgent': return 0.85
      case 'routine': return 0.75
      default: return 0.7
    }
  }
  ...
  private generateOutcomesForCategory(category: string): string[] {
    const outcomeMap: Record<string, string[]> = {
      depression: ['Improved mood', 'Increased energy', 'Better sleep', 'Enhanced motivation'],
      anxiety: ['Reduced worry', 'Improved relaxation', 'Better stress management', 'Increased confidence'],
      trauma: ['Reduced flashbacks', 'Improved emotional regulation', 'Better relationships', 'Increased safety']
    }

    return outcomeMap[category] || ['Symptom improvement', 'Better functioning', 'Enhanced well-being']
  }

  private getRiskConsiderationsForCategory(category: string): string[] {
    const riskMap: Record<string, string[]> = {
      depression: ['Monitor for suicidal ideation', 'Watch for worsening symptoms'],
      anxiety: ['Monitor for panic attacks', 'Avoid overexposure'],
      trauma: ['Risk of re-traumatization', 'Requires trauma-informed approach']
    }
  ...
  private getAssessmentToolsForCategory(category: string): string[] {
    const toolMap: Record<string, string[]> = {
      depression: ['PHQ-9', 'Beck Depression Inventory', 'Hamilton Depression Rating Scale'],
      anxiety: ['GAD-7', 'Beck Anxiety Inventory', 'State-Trait Anxiety Inventory'],
      trauma: ['PCL-5', 'CAPS-5', 'Trauma Symptom Inventory']
    }

    return toolMap[category] || ['Standardized assessment scales', 'Clinical interview']
  }
}
