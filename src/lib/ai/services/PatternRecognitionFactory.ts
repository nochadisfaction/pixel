...
class MockPatternRecognitionService implements PatternRecognitionService {
    ...
    const mockPatterns: PatternRecognitionResult[] = [
      {
        ...
        type: 'behavioral',
        description:
          'Recurring avoidance behavior when discussing family relationships',
        frequency: 3,
        confidence: 0.85,
        ...
        clinicalRelevance: {
          significance: 'high',
          recommendation:
            'Consider exploring family dynamics in future sessions',
  ...
  async comparePatterns(
    ...
    const unique1 = patterns1.filter(
      (p1) => !patterns2.some((p2) => p2.description === p1.description),
    )
    const unique2 = patterns2.filter(
      (p2) => !patterns1.some((p1) => p1.description === p2.description),
    )
