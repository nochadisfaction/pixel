  ...
  describe('checkBeliefConsistency', () => {
    ...
    it('should only check against N recent patient statements', async () => {
      ...
      const history: ConversationMessage[] = [
        { role: 'patient', content: 'This is too old', timestamp: new Date(Date.now() - 200000).toISOString() },
        { role: 'patient', content: oldStatement, timestamp: new Date(Date.now() - 100000).toISOString() },
      ...
      let result = await service.checkBeliefConsistency(profile, newStatementContradictingOld, 2);
      expect(result.isConsistent).toBe(true);

      // Check against last 3 statements (Mondays, Tuesday, coffee) - should be inconsistent
      result = await service.checkBeliefConsistency(profile, newStatementContradictingOld, 3);
      expect(result.isConsistent).toBe(false);
      expect(result.contradictionsFound[0].type).toBe('statement');
