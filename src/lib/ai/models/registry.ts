...
export interface ExtendedAIModel extends AIModel {
  ...
  legacyCapabilities: Array<'chat' | 'sentiment' | 'crisis' | 'response' | 'intervention'>
}
...
const modelDatabase: ExtendedAIModel[] = [
  // OpenAI Models
  {
    ...
    specialization: {
      ...
      useCases: ['therapeutic-conversation', 'crisis-assessment', 'analysis', 'coding'],
      strengths: ['multimodal-understanding', 'complex-reasoning', 'context-retention'],
  ...
  // Anthropic Models
  {
    ...
    specialization: {
      ...
      useCases: ['crisis-intervention', 'deep-analysis', 'therapeutic-conversation'],
      strengths: ['safety-alignment', 'nuanced-understanding', 'large-context', 'ethical-reasoning'],
      limitations: ['cost', 'slower-speed']
    },
    legacyCapabilities: ['chat', 'sentiment', 'crisis', 'response', 'intervention']
  },
  {
    ...
    specialization: {
      ...
      useCases: ['therapeutic-conversation', 'emotion-validation', 'safety-assessment'],
      strengths: ['balanced-performance', 'safety-focused', 'therapeutic-awareness'],
  ...
  // Local/Experimental Models
  {
    ...
    specialization: {
      ...
      useCases: ['therapeutic-conversation', 'mental-health-assessment', 'emotion-analysis'],
      strengths: ['specialized-training', 'privacy-focused', 'local-deployment'],
      limitations: ['smaller-model', 'experimental', 'limited-general-knowledge']
    },
...
export class AIModelRegistry {
  ...
  private initializeRegistry(): void {
    ...
    appLogger.info(`AI Model Registry initialized with ${this.models.size} models across ${this.providerModels.size} providers`)
  }
  ...
  selectOptimalModel(requirements: {
    ...
    if (requirements.maxLatency && candidates.length > 0) {
      candidates = candidates.filter(model =>
        !model.performance || model.performance.averageLatencyMs <= requirements.maxLatency!
      )
    }
    ...
    if (requirements.minReliability && candidates.length > 0) {
      candidates = candidates.filter(model =>
        !model.performance || model.performance.reliabilityScore >= requirements.minReliability!
      )
    }

    if (requirements.contextWindowMin && candidates.length > 0) {
      candidates = candidates.filter(model =>
        model.contextWindow >= requirements.contextWindowMin!
      )
    }
  ...
  private calculateModelScore(model: ExtendedAIModel, requirements: {
    capabilities?: ModelCapability[]
    maxLatency?: number
    maxCostPerToken?: number
    minReliability?: number
    ...
    if (model.performance?.averageLatencyMs) {
      const speedScore = Math.max(0, 20 - (model.performance.averageLatencyMs / 1000) * 2)
      score += speedScore
    }

    // Cost efficiency (20% weight)
    if (model.pricing?.outputTokensPerMillion) {
      const costScore = Math.max(0, 20 - (model.pricing.outputTokensPerMillion / 5000))
      score += costScore
    }
   ...
   */
  updateProviderHealth(provider: string, health: Partial<ProviderHealth>): void {
  ...
  getStatistics(): {
    ...
    return {
      ...
      availableProviders: Array.from(this.providerModels.keys()).filter(p => this.isProviderAvailable(p)).length,
      modelsByProvider,
