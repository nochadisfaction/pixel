...
vi.mock('../../crisis/CrisisSessionFlaggingService', () => ({ // Path based on usage in adapter
  CrisisSessionFlaggingService: vi.fn().mockImplementation(() => ({
...
describe('MentalLLaMAAdapter (Consolidated)', () => {
  ...
  beforeEach(() => {
    mockModelProvider = {
      invoke: vi.fn(),
      getModelConfig: vi.fn().mockReturnValue({ modelId: 'test-model-7b', providerType: 'custom_api' }),
      getModelTier: vi.fn().mockReturnValue('7B'),
      // Add chat if it's part of IModelProvider and used by adapter directly (it's not, adapter uses invoke)
    };
    mockTaskRouter = {
      route: vi.fn(),
      getAvailableAnalyzers: vi.fn().mockReturnValue(['general_mental_health', 'crisis', 'depression']),
    ...
    mockEvidenceService = new EvidenceService(mockModelProvider) as vi.Mocked<EvidenceService>;
    mockExpertGuidanceOrchestrator = new ExpertGuidanceOrchestrator(
      mockEvidenceService,
      mockModelProvider,
      mockCrisisNotifier
    ) as vi.Mocked<ExpertGuidanceOrchestrator>;
    ...
    // Mock methods of the instances
     vi.mocked(mockEvidenceService.extractSupportingEvidence).mockResolvedValue({
      ...
      processingMetadata: { cacheUsed: false, processingTime: 10, evidenceStrength: 'moderate' },
    });
    vi.mocked(mockExpertGuidanceOrchestrator.analyzeWithExpertGuidance).mockResolvedValue({
  ...
  describe('analyzeMentalHealth', () => {
    ...
    it('should use taskRouter for auto_route and process LLM response', async () => {
      ...
      expect(mockTaskRouter.route).toHaveBeenCalledWith({ text: baseParams.text, context: {} });
      ...
      expect(result.supportingEvidence).toEqual(expect.arrayContaining(['feeling down a lot', 'mocked evidence']));
    ...
    it('should handle crisis identified by router with high confidence', async () => {
      ...
      const CrisisSessionFlaggingService = (await import('../../crisis/CrisisSessionFlaggingService')).CrisisSessionFlaggingService;
      const flaggingServiceInstance = vi.mocked(CrisisSessionFlaggingService).mock.results[0]?.value; // Get instance
      expect(flaggingServiceInstance.flagSessionForReview).toHaveBeenCalled();
    ...
    it('should handle LLM confirming crisis', async () => {
        ...
        expect(mockCrisisNotifier.sendCrisisAlert).toHaveBeenCalled();
        const CrisisSessionFlaggingService = (await import('../../crisis/CrisisSessionFlaggingService')).CrisisSessionFlaggingService;
        const flaggingServiceInstance = vi.mocked(CrisisSessionFlaggingService).mock.results[0]?.value;
        expect(flaggingServiceInstance.flagSessionForReview).toHaveBeenCalled();
    });
    ...
    it('should handle LLM response parsing error', async () => {
      ...
      vi.mocked(mockModelProvider.invoke).mockResolvedValue(llmMalformedResponse);
    ...
    it('should handle missing modelProvider gracefully', async () => {
        const mockRoutingDecision: RoutingDecision = {
            targetAnalyzer: 'general_mental_health', confidence: 0.6, isCritical: false, method: 'llm_classification', insights: {}
        };
        vi.mocked(mockTaskRouter.route).mockResolvedValue(mockRoutingDecision);

        const adapterWithoutProvider = new MentalLLaMAAdapter({ ...adapterOptions, modelProvider: undefined });
        ...
        expect(result._failures).toEqual(
            expect.arrayContaining([
              expect.objectContaining({ type: 'configuration', message: 'ModelProvider unavailable for detailed analysis' })
            ])
        );
    });
  });
  ...
  describe('analyzeMentalHealthWithExpertGuidance', () => {
    it('should call expertGuidanceOrchestrator', async () => {
      ...
      const analyzeMentalHealthSpy = vi.spyOn(adapter, 'analyzeMentalHealth').mockResolvedValue(mockBaseAnalysis);


      await adapter.analyzeMentalHealthWithExpertGuidance(text, true, {});

      expect(analyzeMentalHealthSpy).toHaveBeenCalled();
      expect(mockExpertGuidanceOrchestrator.analyzeWithExpertGuidance).toHaveBeenCalledWith(
        text,
    ...
    it('should fallback to base analysis if orchestrator fails', async () => {
        ...
        const mockBaseAnalysis: MentalHealthAnalysisResult = {
            hasMentalHealthIssue: true, mentalHealthCategory: 'anxiety', confidence: 0.6,
            explanation: 'Base fallback explanation', supportingEvidence: [], isCrisis: false,
            timestamp: new Date().toISOString(), modelTier: '7B'
        };
        const analyzeMentalHealthSpy = vi.spyOn(adapter, 'analyzeMentalHealth').mockResolvedValue(mockBaseAnalysis);
        const orchestratorError = new Error("Orchestrator failed");
        vi.mocked(mockExpertGuidanceOrchestrator.analyzeWithExpertGuidance).mockRejectedValue(orchestratorError);
        ...
        expect(result._failures).toEqual(
            expect.arrayContaining([
              expect.objectContaining({ type: 'orchestration', error: orchestratorError })
            ])
        );
        analyzeMentalHealthSpy.mockRestore();
    });
  ...
  describe('evaluateExplanationQuality', () => {
    it('should call modelProvider.invoke with correct prompt for quality evaluation', async () => {
      ...
      vi.mocked(mockModelProvider.invoke).mockResolvedValue(mockLLMQualityResponse);
      ...
      expect(mockModelProvider.invoke).toHaveBeenCalledWith(
        expect.arrayContaining([
          expect.objectContaining({ role: 'system', content: expect.stringContaining('evaluates the quality of explanations') }),
          expect.objectContaining({ role: 'user', content: expect.stringContaining(explanation) && expect.stringContaining(textContext) }),
        ]),
        { temperature: 0.2, max_tokens: 200 }
      );
      expect(result.overall).toBe(0.8);
      expect(result.message).toBe('Good quality.');
    });
    ...
    it('should return default poor metrics if modelProvider is undefined', async () => {
        ...
        expect(result.message).toContain("ModelProvider not configured");
    });
  });
  ...
  describe('EvidenceService wrapper methods', () => {
    it('extractDetailedEvidence should call evidenceService', async () => {
      ...
      expect(mockEvidenceService.extractSupportingEvidence).toHaveBeenCalledWith('text', 'category', undefined, undefined);
    });
