...
const DEPRESSION_KEYWORDS = [
  'depressed', 'hopeless', 'worthless', 'empty', 'sad all the time',
  'no energy', 'sleep all day', 'nothing matters'
]

const ANXIETY_KEYWORDS = [
  'panic', 'anxious', 'worried', 'scared', 'racing heart',
  'can\'t breathe', 'overwhelming', 'nervous'
]
...
export class MentalHealthTaskRouter implements IMentalHealthTaskRouter {
  ...
  async route(input: RoutingInput): Promise<RoutingDecision> {
    ...
    try {
      const llmResult = await this.llmInvoker([
        { role: 'system', content: 'Classify this mental health text into: crisis, depression, anxiety, or general. Respond with just the category.' },
  ...
  getAvailableAnalyzers(): string[] {
    return ['crisis', 'depression', 'anxiety', 'general_mental_health', 'unknown']
  }
