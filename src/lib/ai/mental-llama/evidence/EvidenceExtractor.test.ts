...
describe('Evidence Extraction System', () => {
  describe('EvidenceExtractor', () => {
    it('should extract depression evidence correctly', async () => {
      ...
      const result = await extractor.extractEvidence(depressionText, 'depression');
    ...
    it('should extract crisis evidence with high severity', async () => {
      ...
      const criticalEvidence = result.evidenceItems.filter(item => 
  ...
  describe('EvidenceService', () => {
    ...
    it('should extract crisis evidence with detailed breakdown', async () => {
      ...
      expect(crisisEvidence.immediateRiskIndicators.length).toBeGreaterThanOrEqual(0);
    });
    ...
    it('should enhance evidence with LLM integration', async () => {
      ...
      expect(result.evidenceItems.length).toBeGreaterThan(0);
      expect(result.detailedEvidence.extractionMetadata.processingTime).toBeGreaterThan(0);
    });
    ...
    it('should handle real-world complex scenarios', async () => {
      ...
      expect(result.detailedEvidence.summary.riskIndicatorCount).toBeGreaterThanOrEqual(0);
      expect(result.detailedEvidence.summary.supportiveFactorCount).toBeGreaterThanOrEqual(0);
      expect(result.detailedEvidence.summary.highConfidenceCount).toBeGreaterThanOrEqual(0);
      
      // Verify we have meaningful evidence items
      expect(result.evidenceItems.every(item => typeof item === 'string')).toBe(true);
