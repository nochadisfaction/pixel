...
import { EvidenceExtractor, type EvidenceItem, type EvidenceExtractionResult } from './EvidenceExtractor';
import type { 
  IModelProvider, 
...
export class EvidenceService {
  ...
  async extractSupportingEvidence(
    ...
    try {
      ...
      if (!detailedEvidence) {
        detailedEvidence = await this.extractor.extractEvidence(text, category, baseAnalysis);
      ...
      const evidenceItems = this.convertToStringArray(detailedEvidence.evidenceItems);
  ...
  async extractCrisisEvidence(
    ...
    try {
      const evidenceResult = await this.extractor.extractEvidence(text, 'crisis', baseAnalysis);
  ...
  assessEvidenceQuality(evidenceResult: EvidenceExtractionResult): {
    ...
    if (qualityMetrics.completeness < 0.5) {
      recommendations.push('Consider requesting more detailed information from the user');
    }
    if (qualityMetrics.specificity < 0.5) {
      recommendations.push('Look for more specific behavioral or symptom indicators');
    }
    if (summary.riskIndicatorCount > 0) {
      recommendations.push('Prioritize immediate risk assessment and safety planning');
    }
    if (summary.supportiveFactorCount > 0) {
      recommendations.push('Leverage identified protective factors in treatment planning');
    }
  ...
  private extractContextualInsights(
    ...
    if (decisionInsights?.['matchedKeywords'] && Array.isArray(decisionInsights['matchedKeywords'])) {
      insights.push(`Routing keywords: ${(decisionInsights['matchedKeywords'] as string[]).join(', ')}`);
    }
 ...
 */
export function evidenceToStringArray(evidenceResult: EvidenceExtractionResult): string[] {
