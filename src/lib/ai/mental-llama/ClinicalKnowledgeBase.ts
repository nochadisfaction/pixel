...
export class ClinicalKnowledgeBase {
   ...
   */
  public assessRiskFactors(text: string, category: string, baseAnalysis: MentalHealthAnalysisResult) {
    ...
    const categoryRiskKeywords: Record<string, {
      critical: string[];
      high: string[];
      moderate: string[];
      low: string[];
      ...
      depression: {
        critical: ['suicide', 'kill myself', 'end it all', 'no point living', 'better off dead'],
        high: ['hopeless', 'worthless', 'burden', 'trapped', 'empty', 'numb'],
        moderate: ['sad', 'down', 'depressed', 'lonely', 'tired'],
        low: ['blue', 'upset', 'disappointed'],
      },
      anxiety: {
        critical: ['panic attack', 'can\'t breathe', 'heart racing', 'going to die'],
      ...
      crisis: {
        critical: ['suicide', 'kill myself', 'end it all', 'hurt myself', 'weapon'],
   ...
   */
  public getClinicalContext(category: string, baseAnalysis: MentalHealthAnalysisResult) {
  ...
  private enhanceContextWithAnalysis(
    ...
    if (baseAnalysis.isCrisis) {
      ...
      if (category === 'depression') {
        enhancedContext.relevantDiagnoses.push('Major Depressive Disorder with Suicidal Ideation');
        enhancedContext.contraindications.push('Delay in crisis intervention contraindicated');
      }
    }
    ...
    this.addCategorySpecificContextEnhancements(enhancedContext, baseAnalysis, category);
  ...
  private addCategorySpecificContextEnhancements(
    ...
    switch (category) {
      case 'depression':
        ...
        if (baseAnalysis.hasMentalHealthIssue) {
          context.relevantDiagnoses.push('Depressive Disorder - Further Assessment Needed');
          context.specialConsiderations.push(
      ...
      case 'anxiety':
        if (baseAnalysis.confidence > 0.8 && baseAnalysis.hasMentalHealthIssue) {
          context.relevantDiagnoses.push('Anxiety Disorder with Significant Distress');
          context.specialConsiderations.push(
        ...
        context.contraindications.push('Avoid benzodiazepines for long-term treatment');
        break;
