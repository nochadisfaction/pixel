...
import type { LLMInvoker, LLMInvocationOptions } from './types/mentalLLaMATypes';
...
export class MentalLLaMAFactory {
  ...
  static async createFromEnv(): Promise<{
    ...
    return MentalLLaMAFactory.create(config);
  }
}
...
export async function createMentalLLaMAFromEnv(): Promise<{
  ...
  logger.info('Creating MentalLLaMA components from environment configuration...')

  try {
    const result = await MentalLLaMAFactory.createFromEnv()
