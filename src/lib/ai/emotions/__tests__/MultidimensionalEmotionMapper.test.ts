...
import type { EmotionVector, DimensionalMap, EmotionAnalysis, EmotionDimensions } from '../types';
...
describe('MultidimensionalEmotionMapper', () => {
  ...
  describe('findPrimaryEmotion', () => {
    ...
    it('should return a valid key if all emotions are 0', () => {
        const emotions: EmotionVector = { joy: 0, sadness: 0, anger: 0, fear: 0, surprise: 0, disgust: 0, trust: 0, anticipation: 0 };
        const result = mapper['findPrimaryEmotion'](emotions);
  ...
  describe('mapEmotionsToDimensions', () => {
    it('should map emotions to dimensions correctly', () => {
      const emotionsInput = { joy: 0.8, sadness: 0.1, anger: 0.2, fear: 0.0, surprise: 0.3, disgust: 0.0, trust: 0.5, anticipation: 0.4 };
      ...
      // Valence
      const expectedValencePositive = emotionsInput.joy + emotionsInput.trust + emotionsInput.surprise * 0.5;
      const expectedValenceNegative = emotionsInput.sadness + emotionsInput.anger + emotionsInput.fear + emotionsInput.disgust;
      const expectedValence = Math.tanh((expectedValencePositive - expectedValenceNegative) / 2);
      expect(dimensionalMap.dimensions.valence).toBeCloseTo(expectedValence, 9);

      // Arousal
      const expectedArousalHigh = emotionsInput.anger + emotionsInput.fear + emotionsInput.surprise + emotionsInput.joy * 0.7;
      const expectedArousalLow = emotionsInput.sadness + emotionsInput.trust + emotionsInput.disgust * 0.5;
      const expectedArousalCalc = (expectedArousalHigh - expectedArousalLow * 0.3) / 2;
      ...
      // Dominance
      const expectedDominant = emotionsInput.anger + emotionsInput.trust + emotionsInput.anticipation;
      const expectedSubmissive = emotionsInput.fear + emotionsInput.sadness + emotionsInput.surprise * 0.5;
      const expectedDominance = Math.tanh((expectedDominant - expectedSubmissive) / 2);
      expect(dimensionalMap.dimensions.dominance).toBeCloseTo(expectedDominance, 9);

      expect(dimensionalMap.primaryEmotion).toBe('joy');

      // Intensity
      const expectedIntensitySum = Object.values(emotionsInput).reduce((acc, val) => acc + val, 0);
  ...
  describe('smoothDimensions', () => {
    ...
    it('should smooth dimensions using exponential moving average logic', () => {
      ...
      expectDimensionsCloseTo(smoothedMaps[1]!.dimensions, {
        valence: alpha * mapsToSmooth[0]!.dimensions.valence + (1 - alpha) * mapsToSmooth[1]!.dimensions.valence,
        arousal: alpha * mapsToSmooth[0]!.dimensions.arousal + (1 - alpha) * mapsToSmooth[1]!.dimensions.arousal,
        dominance: alpha * mapsToSmooth[0]!.dimensions.dominance + (1 - alpha) * mapsToSmooth[1]!.dimensions.dominance,
      });
      expect(smoothedMaps[1]!.primaryEmotion).toBe(mapsToSmooth[1]!.primaryEmotion); // Other fields preserved

      // Third map smoothed based on the previously smoothed second map's values
      expectDimensionsCloseTo(smoothedMaps[2]!.dimensions, {
        valence: alpha * smoothedMaps[1]!.dimensions.valence + (1 - alpha) * mapsToSmooth[2]!.dimensions.valence,
        arousal: alpha * smoothedMaps[1]!.dimensions.arousal + (1 - alpha) * mapsToSmooth[2]!.dimensions.arousal,
        dominance: alpha * smoothedMaps[1]!.dimensions.dominance + (1 - alpha) * mapsToSmooth[2]!.dimensions.dominance,
      });
       expect(smoothedMaps[2]!.primaryEmotion).toBe(mapsToSmooth[2]!.primaryEmotion);
    });
  });
});
