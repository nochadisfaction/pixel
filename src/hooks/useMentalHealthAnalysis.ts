...
export const useMentalHealthAnalysis = () => {
  const analyzeMessage = useCallback(async (message: string): Promise<MentalHealthAnalysisResult> => {
    ...
    const anxietyKeywords = ['anxiety', 'anxious', 'worried', 'stress', 'nervous', 'panic']
    const depressionKeywords = ['sad', 'depressed', 'hopeless', 'worthless', 'suicide', 'death']
    const riskKeywords = ['hurt', 'harm', 'kill', 'end it all', 'suicide', 'die']
    
    const lowerMessage = message.toLowerCase()
    
    // Calculate basic scores
    const anxietyScore = anxietyKeywords.filter(keyword => lowerMessage.includes(keyword)).length
    const depressionScore = depressionKeywords.filter(keyword => lowerMessage.includes(keyword)).length
    const riskScore = riskKeywords.filter(keyword => lowerMessage.includes(keyword)).length
    ...
    if (anxietyScore > 0) {
      supportingEvidence.push(`Detected ${anxietyScore} anxiety-related term(s)`)
    }
    if (depressionScore > 0) {
      supportingEvidence.push(`Detected ${depressionScore} depression-related term(s)`)
    }
    if (riskScore > 0) {
