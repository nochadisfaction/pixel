...
import type { BiasAnalysisResults, CounterfactualScenario, HistoricalComparison } from '../../../../lib/types/bias-detection'
...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    const filteredExportData = {
      ...
      counterfactualScenarios: includeComponents.counterfactual ? exportData.counterfactualScenarios : undefined,
      historicalComparison: includeComponents.historical ? exportData.historicalComparison : undefined,
      metadata: {
        ...exportData.metadata,
        includedComponents: Object.keys(includeComponents).filter(key => includeComponents[key as keyof typeof includeComponents])
      }
    }
...
function convertToCSV(exportData: Record<string, unknown>): string {
  ...
  if (exportData.analysis) {
    ...
    csvRows.push(`Analysis,Overall Bias Score,${analysis.overallBiasScore},${analysis.alertLevel}`)
    ...
    csvRows.push(`Preprocessing,Gender Bias,${layers.preprocessing.linguisticBias.genderBiasScore},`)
    csvRows.push(`Preprocessing,Racial Bias,${layers.preprocessing.linguisticBias.racialBiasScore},`)
    csvRows.push(`Preprocessing,Age Bias,${layers.preprocessing.linguisticBias.ageBiasScore},`)
    csvRows.push(`Preprocessing,Cultural Bias,${layers.preprocessing.linguisticBias.culturalBiasScore},`)
    
    csvRows.push(`Model,Demographic Parity,${layers.modelLevel.fairnessMetrics.demographicParity},`)
    csvRows.push(`Model,Equalized Odds,${layers.modelLevel.fairnessMetrics.equalizedOdds},`)
    csvRows.push(`Model,Calibration,${layers.modelLevel.fairnessMetrics.calibration},`)
    
    csvRows.push(`Interactive,Scenarios Analyzed,${layers.interactive.counterfactualAnalysis.scenariosAnalyzed},`)
    csvRows.push(`Interactive,Bias Detected,${layers.interactive.counterfactualAnalysis.biasDetected},`)
    csvRows.push(`Interactive,Consistency Score,${layers.interactive.counterfactualAnalysis.consistencyScore},`)
  }
  
  // Counterfactual scenarios
  if (exportData.counterfactualScenarios) {
    const scenarios = exportData.counterfactualScenarios as Array<Record<string, unknown>>
    scenarios.forEach((scenario, index) => {
      csvRows.push(`Counterfactual,Scenario ${index + 1},${scenario.expectedBiasReduction},${scenario.change}`)
      csvRows.push(`Counterfactual,Likelihood ${index + 1},${scenario.likelihood},${scenario.description}`)
    })
  }
...
export const GET: APIRoute = async () => {
  return new Response(JSON.stringify({
    ...
    usage: {
      ...
      optionalFields: ['counterfactualScenarios', 'historicalComparison', 'format', 'includeComponents'],
      examples: {
        json: { format: 'json' },
        csv: { format: 'csv' },
        text: { format: 'txt' }
