...
export const GET: APIRoute = async ({ url }) => {
  try {
    ...
    if (category) {
      ...
      if (scenarios.length === 0) {
        return new Response(JSON.stringify({
          ...
          availableCategories: [...new Set(PRESET_SCENARIOS.map(s => s.category))]
        }), {
          status: 404,
          headers: { 'Content-Type': 'application/json' }
        })
    ...
    if (riskLevel) {
      const scenarios = getPresetScenariosByRiskLevel(riskLevel as 'low' | 'medium' | 'high' | 'critical')
      if (scenarios.length === 0) {
        return new Response(JSON.stringify({
          error: 'No scenarios found for risk level',
          availableRiskLevels: [...new Set(PRESET_SCENARIOS.map(s => s.riskLevel))]
        }), {
          status: 404,
          headers: { 'Content-Type': 'application/json' }
        })
    ...
    const categoryCounts = PRESET_SCENARIOS.reduce((acc, scenario) => {
      acc[scenario.category] = (acc[scenario.category] || 0) + 1
      return acc
    }, {} as Record<string, number>)

    const riskLevelCounts = PRESET_SCENARIOS.reduce((acc, scenario) => {
