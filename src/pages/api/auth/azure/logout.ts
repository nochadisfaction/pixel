...
export const GET: APIRoute = async ({ url, redirect, request }) => {
  try {
    ...
    if (userId) {
      logoutUrl = await azureSupabaseIntegration.signOut(userId, postLogoutRedirectUri)
    } else {
      // If no user session, just redirect to Azure AD logout
      const azureConfig = (await import('../../../../config/azure.config')).azureConfig
      logoutUrl = azureConfig.auth.getOAuthConfig().authority + 
...
export const POST: APIRoute = async ({ request, url }) => {
  try {
    ...
    const postLogoutRedirectUri = post_logout_redirect_uri || `${url.origin}/auth/logged-out`
