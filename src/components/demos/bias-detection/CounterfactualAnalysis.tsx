...
import type { CounterfactualScenario, SessionData } from '../../../lib/types/bias-detection'
...
export const CounterfactualAnalysis: React.FC<CounterfactualAnalysisProps> = ({
  ...
  const [selectedScenario, setSelectedScenario] = useState<CounterfactualScenario | null>(null)
  const [sortBy, setSortBy] = useState<'likelihood' | 'impact' | 'change'>('likelihood')
  ...
  const sortedScenarios = [...scenarios].sort((a, b) => {
    switch (sortBy) {
      case 'likelihood': {
        ...
        return likelihoodOrder[b.likelihood as keyof typeof likelihoodOrder] - 
    ...
    <div className="counterfactual-analysis space-y-6">
      ...
      {originalSession && (
        <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
          <h4 className="font-medium text-gray-900 mb-2">Original Session Context</h4>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            <div>
              <span className="text-gray-500">Age:</span>
              <span className="ml-1 font-medium">{originalSession.demographics.age}</span>
            </div>
            <div>
              <span className="text-gray-500">Gender:</span>
              <span className="ml-1 font-medium">{originalSession.demographics.gender}</span>
            </div>
            <div>
              <span className="text-gray-500">Ethnicity:</span>
              <span className="ml-1 font-medium">{originalSession.demographics.ethnicity}</span>
            </div>
            <div>
              <span className="text-gray-500">Language:</span>
              <span className="ml-1 font-medium">{originalSession.demographics.primaryLanguage}</span>
            </div>
          </div>
        </div>
      )}
          ...
          >
            ...
            <div className="flex items-start justify-between mb-3">
              ...
              <div className="flex items-center space-x-3 ml-4">
                {/* Likelihood Badge */}
                <span className={`px-2 py-1 rounded-full text-xs font-medium border ${getLikelihoodStyle(scenario.likelihood)}`}>
            ...
            {selectedScenario === scenario && (
              <div className="border-t pt-4 mt-4 space-y-4">
                    ...
                    <div className="bg-white rounded border p-3">
                      ...
                      {scenario.change.includes('Language') && (
                        <div className="mt-2 text-xs text-purple-600">
                          This scenario examines the impact of therapeutic language choices on bias patterns.
                        </div>
                      )}
                      {scenario.change.includes('Cultural') && (
                        <div className="mt-2 text-xs text-green-600">
                          This scenario investigates cultural sensitivity in therapeutic approaches.
                        </div>
                      )}
                    </div>
                  </div>
                ...
                <div>
                  <h5 className="font-medium text-gray-900 mb-2">Implementation Feasibility</h5>
                  <div className="bg-white rounded border p-3">
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-gray-600">Likelihood of Success</span>
                      <span className={`px-2 py-1 rounded text-xs font-medium ${getLikelihoodStyle(scenario.likelihood)}`}>
                ...
                <div>
                  ...
                  <div className="bg-blue-50 rounded border border-blue-200 p-3">
                    <ul className="text-sm text-blue-800 space-y-1">
                      {scenario.change.includes('Demographics') && (
                        <>
                          <li>• Review demographic assumptions in assessment tools</li>
                          <li>• Implement culturally responsive therapeutic approaches</li>
                        </>
                      )}
                      {scenario.change.includes('Language') && (
                        <>
                      ...
                      <li>• Collect feedback from diverse client populations</li>
                    </ul>
                  </div>
                </div>
              </div>
      ...
      {scenarios.length === 0 && (
        <div className="text-center py-8 text-gray-500">
          ...
          <p className="text-sm mt-1">Run a bias analysis first to generate scenarios</p>
        </div>
      )}
    </div>
  )
