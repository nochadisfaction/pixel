...
export const ExportControls: React.FC<ExportControlsProps> = ({
  ...
  const getExportSizeEstimate = () => {
    ...
    if (includeComponents.counterfactual) size += counterfactualScenarios.length * 2
    if (includeComponents.historical) size += 5
    if (includeComponents.recommendations) size += analysisResults.recommendations.length * 0.5
  ...
  const exportAsCSV = () => {
    ...
    if (includeComponents.analysis) {
      csvData.push(`Overall Bias Score,${analysisResults.overallBiasScore},Analysis`)
      csvData.push(`Alert Level,${analysisResults.alertLevel},Analysis`)
      csvData.push(`Confidence,${analysisResults.confidence},Analysis`)
      
      // Layer results
      csvData.push(`Gender Bias,${analysisResults.layerResults.preprocessing.linguisticBias.genderBiasScore},Preprocessing`)
      csvData.push(`Racial Bias,${analysisResults.layerResults.preprocessing.linguisticBias.racialBiasScore},Preprocessing`)
      csvData.push(`Age Bias,${analysisResults.layerResults.preprocessing.linguisticBias.ageBiasScore},Preprocessing`)
      csvData.push(`Cultural Bias,${analysisResults.layerResults.preprocessing.linguisticBias.culturalBiasScore},Preprocessing`)
    }
    
    // Counterfactual data
    if (includeComponents.counterfactual) {
      counterfactualScenarios.forEach((scenario, index) => {
        csvData.push(`Counterfactual ${index + 1},${scenario.expectedBiasReduction},Counterfactual`)
        csvData.push(`Likelihood ${index + 1},${scenario.likelihood},Counterfactual`)
      })
    }
    
    // Historical data
    if (includeComponents.historical && historicalComparison) {
      csvData.push(`30-Day Average,${historicalComparison.thirtyDayAverage},Historical`)
      csvData.push(`Percentile Rank,${historicalComparison.percentileRank},Historical`)
      csvData.push(`7-Day Trend,${historicalComparison.sevenDayTrend},Historical`)
    }
    ...
    <div className="export-controls space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-600">
          Download comprehensive bias analysis results for reporting and compliance
        </p>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        ...
        <div className="space-y-3">
          ...
          <label className="flex items-center">
            ...
            />
            <div className="flex-1">
              <span className="font-medium text-gray-900">Counterfactual Scenarios</span>
              <p className="text-sm text-gray-600">
          ...
          <label className="flex items-center">
            ...
            />
            <div className="flex-1">
              <span className={`font-medium ${historicalComparison ? 'text-gray-900' : 'text-gray-400'}`}>
                Historical Comparison
              </span>
              <p className={`text-sm ${historicalComparison ? 'text-gray-600' : 'text-gray-400'}`}>
          ...
          <label className="flex items-center">
            ...
            />
            <div className="flex-1">
              <span className="font-medium text-gray-900">Demographics Context</span>
              <p className="text-sm text-gray-600">
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <div className="text-sm text-gray-600">
              ...
              Size: ~{getExportSizeEstimate().toFixed(1)} KB • 
              Components: {Object.values(includeComponents).filter(Boolean).length}/5
            </div>
          </div>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        ...
        <ul className="text-sm text-blue-800 space-y-1">
          ...
          <li>• Consider removing demographic information for anonymized reports</li>
