...
export const BiasDetectionDemo: React.FC<BiasDetectionDemoProps> = ({
  ...
  // Analysis function
  const performBiasAnalysis = useCallback(async (data: SessionData) => {
    setIsAnalyzing(true)
    setError(null)
    ...
    try {
      ...
      const results: BiasAnalysisResults = {
        ...
        alertLevel: biasFactors.overall >= 0.8 ? 'critical' : 
                   biasFactors.overall >= 0.6 ? 'high' :
                   biasFactors.overall >= 0.4 ? 'medium' : 'low',
        ...
        recommendations: generateRecommendations(biasFactors, data.demographics),
  ...
  const handleSessionSubmit = useCallback((data: Omit<SessionData, 'sessionId' | 'timestamp'>) => {
  ...
  // Tab navigation
  const tabs = [
    { id: 'analysis' as const, label: 'Bias Analysis', disabled: !analysisResults },
    { id: 'counterfactual' as const, label: 'Counterfactual Analysis', disabled: !counterfactualScenarios.length },
    { id: 'historical' as const, label: 'Historical Progress', disabled: !historicalComparison },
    ...
    <div className={`bias-detection-demo ${className}`}>
      ...
      <div className="demo-header mb-6">
        ...
        <p className="text-gray-600">
          Analyze therapeutic conversations for bias patterns across multiple dimensions
        </p>
      </div>
      ...
      {isAnalyzing && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-6 mb-6">
          <div className="flex items-center">
            ...
            <span className="text-blue-800 font-medium">Analyzing bias patterns...</span>
          </div>
        </div>
      )}
      ...
      {analysisResults && (
        <div className="bg-white rounded-lg shadow-sm border border-gray-200">
          ...
          <div className="p-6">
            ...
            {activeTab === 'counterfactual' && counterfactualScenarios.length > 0 && (
              <CounterfactualAnalysis
