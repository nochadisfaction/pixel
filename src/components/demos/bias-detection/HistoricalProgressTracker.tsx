...
export const HistoricalProgressTracker: React.FC<HistoricalProgressTrackerProps> = ({
  comparison,
  currentScore
}) => {
    ...
    <div className="historical-progress-tracker space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-4">
          <div className="text-center">
            ...
            <div className={`text-sm font-medium ${isImprovement ? 'text-green-600' : 'text-red-600'}`}>
              {isImprovement ? '↓' : '↑'} {formatPercentage(Math.abs(scoreDifference))}
        ...
        <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
          <div className="text-center">
            <div className={`text-2xl font-bold mb-1 ${getPercentileColor(comparison.percentileRank)}`}>
              {comparison.percentileRank}th
            </div>
            <div className="text-sm text-purple-800 mb-2">Percentile</div>
            <div className="text-xs text-purple-600">
              {comparison.percentileRank >= 80 ? 'Excellent' :
               comparison.percentileRank >= 60 ? 'Good' :
               comparison.percentileRank >= 40 ? 'Fair' : 'Needs Improvement'}
            </div>
          </div>
        </div>

        {/* 7-Day Trend */}
        <div className={`border rounded-lg p-4 ${getTrendStyle(comparison.sevenDayTrend)}`}>
          <div className="text-center">
            <div className="flex items-center justify-center mb-2">
              {getTrendIcon(comparison.sevenDayTrend)}
              <span className="ml-2 text-lg font-bold">
                {comparison.sevenDayTrend.charAt(0).toUpperCase() + comparison.sevenDayTrend.slice(1)}
              </span>
            </div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        ...
        <div className="space-y-4">
          <div>
            <div className="flex justify-between items-center mb-2">
              ...
              <span className="text-sm font-bold text-gray-900">{formatPercentage(currentScore)}</span>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-3">
              <div
                className={`h-3 rounded-full ${
                  currentScore >= 0.8 ? 'bg-red-500' :
                  currentScore >= 0.6 ? 'bg-orange-500' :
                  currentScore >= 0.4 ? 'bg-yellow-500' : 'bg-green-500'
                }`}
          ...
          <div>
            <div className="flex justify-between items-center mb-2">
              <span className="text-sm font-medium text-gray-700">30-Day Average</span>
              <span className="text-sm font-bold text-blue-600">{formatPercentage(comparison.thirtyDayAverage)}</span>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-3">
              <div
        ...
        <div className="mt-6 p-4 bg-gray-50 rounded-lg">
          ...
          <ul className="text-sm text-gray-700 space-y-1">
            {isImprovement ? (
              <>
                <li className="flex items-start">
                  ...
                  Current session shows {formatPercentage(Math.abs(scoreDifference))} improvement over historical average
                </li>
                <li className="flex items-start">
                  <span className="text-green-500 mr-2">✓</span>
              ...
              <>
                <li className="flex items-start">
                  ...
                  Current session shows {formatPercentage(Math.abs(scoreDifference))} higher bias than historical average
                </li>
                <li className="flex items-start">
            ...
            <li className="flex items-start">
              <span className="text-blue-500 mr-2">ℹ</span>
              You&apos;re performing better than {comparison.percentileRank}% of similar sessions
            </li>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
        <h4 className="font-semibold text-blue-900 mb-4">Historical Recommendations</h4>
        <div className="space-y-3">
          {comparison.percentileRank < 50 && (
            <div className="flex items-start">
              ...
              <div>
                <div className="font-medium text-blue-900">Focus on Improvement</div>
                <div className="text-sm text-blue-800">
                  Your current performance is below the median. Consider additional bias awareness training.
                </div>
              </div>
            </div>
          )}
          ...
          {comparison.sevenDayTrend === 'worsening' && (
            <div className="flex items-start">
              ...
              <div>
                <div className="font-medium text-blue-900">Address Recent Decline</div>
                <div className="text-sm text-blue-800">
                  Recent trend shows increasing bias. Review recent sessions and identify potential causes.
                </div>
              </div>
            </div>
          )}
          ...
          {comparison.percentileRank >= 80 && (
            <div className="flex items-start">
              ...
              <div>
                <div className="font-medium text-blue-900">Excellent Performance</div>
                <div className="text-sm text-blue-800">
                  You&apos;re performing in the top 20%. Consider mentoring others or sharing best practices.
                </div>
              </div>
            </div>
          )}
          ...
          <div className="flex items-start">
            ...
            <div>
              <div className="font-medium text-blue-900">Continue Monitoring</div>
              <div className="text-sm text-blue-800">
                Regular analysis helps maintain awareness and track long-term progress patterns.
              </div>
            </div>
          </div>
        </div>
