...
describe('Pipeline Service Integration Tests', () => {
  ...
  describe('Data Ingestion to Validation Pipeline', () => {
    it('passes processed data from ingestion to validation', async () => {
      ...
      const file = new File(['{"content": "test psychology data"}'], 'test.json', { 
  ...
  describe('Validation to Category Balancing Pipeline', () => {
    it('updates category balance based on validation results', async () => {
      ...
      const textInput = screen.getByPlaceholderText(/Enter psychology content/);
      fireEvent.change(textInput, { 
        target: { value: 'Patient exhibits symptoms of anxiety disorder requiring therapeutic intervention' } 
      });
  ...
  describe('End-to-End Pipeline Integration', () => {
    it('processes data through complete pipeline', async () => {
      ...
      const file = new File([
        JSON.stringify({
          content: 'Clinical assessment shows patient with generalized anxiety disorder',
          category: 'anxiety-disorders'
        })
      ], 'clinical-data.json', { type: 'application/json' });
      ...
      const textInput = screen.getByPlaceholderText(/Enter psychology content/);
      fireEvent.change(textInput, { 
        target: { value: 'Clinical assessment shows patient with generalized anxiety disorder' } 
      });
  ...
  describe('Cross-Component Data Flow', () => {
    it('maintains data integrity across component boundaries', async () => {
      ...
      const { rerender } = render(
        <CategoryBalancingDemo onBalanceUpdate={(categories, metrics) => {
      ...
      rerender(<ResultsExportDemo pipelineData={{
        totalItems: 1000,
    ...
    it('handles component state synchronization', async () => {
      ...
      render(<CategoryBalancingDemo onBalanceUpdate={(categories, metrics) => {
