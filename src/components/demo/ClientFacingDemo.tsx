...
const ClientFacingDemo: React.FC = () => {
  ...
  const sampleFiles = [
    { name: 'anxiety-case-studies.json', size: '2.4 MB', type: 'JSON', status: 'processed' },
    { name: 'therapy-session-notes.csv', size: '1.8 MB', type: 'CSV', status: 'processed' },
    { name: 'clinical-assessments.txt', size: '956 KB', type: 'TXT', status: 'processed' }
  ];

  const validationResults = [
    { category: 'Clinical Accuracy', score: 94, status: 'excellent', color: 'text-green-400' },
    { category: 'Ethical Compliance', score: 98, status: 'excellent', color: 'text-green-400' },
    { category: 'Content Quality', score: 91, status: 'good', color: 'text-blue-400' },
    { category: 'Format Consistency', score: 88, status: 'good', color: 'text-blue-400' }
  ];

  const categoryBalance = [
    { name: 'Anxiety Disorders', percentage: 30, target: 30, color: 'bg-purple-500' },
    { name: 'Mood Disorders', percentage: 25, target: 25, color: 'bg-blue-500' },
    { name: 'Trauma & PTSD', percentage: 20, target: 20, color: 'bg-green-500' },
    { name: 'Personality Disorders', percentage: 15, target: 15, color: 'bg-yellow-500' },
          ...
          {demoSteps.map((step, index) => (
            <div key={step.id} className="flex flex-col items-center flex-1">
              <div className={`w-12 h-12 rounded-full flex items-center justify-center mb-2 ${
                ...
                step.status === 'processing' ? 'bg-blue-600' : 'bg-gray-600'
              }`}>
          ...
          <Card className="bg-slate-800 border-slate-700">
            ...
            <CardContent>
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  <h4 className="text-lg font-semibold text-white mb-4">Sample Files Processed</h4>
                      ...
                      <div key={index} className="flex items-center justify-between p-3 bg-slate-700 rounded-lg">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="text-gray-400 text-xs">{file.size} • {file.type}</div>
                          </div>
                        </div>
                ...
                <div>
                  <h4 className="text-lg font-semibold text-white mb-4">Processing Statistics</h4>
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="bg-slate-700 rounded-lg p-4 text-center">
                      ...
                      <div className="text-sm text-gray-400">Items Extracted</div>
                    </div>
                    <div className="bg-slate-700 rounded-lg p-4 text-center">
        ...
        <TabsContent value="validate" className="mt-6">
          <Card className="bg-slate-800 border-slate-700">
            ...
            <CardContent>
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  <h4 className="text-lg font-semibold text-white mb-4">Validation Categories</h4>
                  <div className="space-y-4">
                    {validationResults.map((result, index) => (
                      <div key={index} className="bg-slate-700 rounded-lg p-4">
                        <div className="flex justify-between items-center mb-2">
                          <span className="text-white font-medium">{result.category}</span>
                          <span className={`font-bold ${result.color}`}>{result.score}%</span>
                        </div>
                ...
                <div>
                  <h4 className="text-lg font-semibold text-white mb-4">Sample Validation</h4>
                  <div className="bg-slate-700 rounded-lg p-4 mb-4">
                    <h5 className="text-purple-400 font-medium mb-2">Input Content</h5>
                    <div className="bg-slate-900 rounded p-3 text-sm text-gray-300">
                      "Client presents with persistent worry, restlessness, and difficulty concentrating for the past 6 months. Symptoms interfere with work performance and social relationships..."
                    </div>
                  </div>
                  
                  <div className="bg-slate-700 rounded-lg p-4">
                    <h5 className="text-green-400 font-medium mb-2">AI Analysis</h5>
                    <div className="space-y-2 text-sm">
                      <div className="flex items-center gap-2">
                        <CheckCircle className="w-4 h-4 text-green-400" />
                        <span className="text-gray-300">Clinical terminology accurate</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <CheckCircle className="w-4 h-4 text-green-400" />
                        <span className="text-gray-300">Ethical guidelines followed</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <CheckCircle className="w-4 h-4 text-green-400" />
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-gray-300">Consider adding duration specificity</span>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <Card className="bg-slate-800 border-slate-700">
            ...
            <CardContent>
              <div className="grid md:grid-cols-2 gap-6">
                    ...
                    {categoryBalance.map((category, index) => (
                      <div key={index} className="bg-slate-700 rounded-lg p-4">
                        <div className="flex justify-between items-center mb-2">
                          ...
                          <div className="flex items-center gap-2">
                            <span className="text-gray-400 text-sm">Target: {category.target}%</span>
                        ...
                        <div className="flex justify-between items-center mt-2">
                          <span className="text-xs text-gray-400">
                            {Math.abs(category.percentage - category.target) === 0 ? 'Perfect' : 
                             Math.abs(category.percentage - category.target) <= 2 ? 'Excellent' : 'Good'}
                          </span>
                          <span className="text-xs text-gray-400">
                            {category.percentage >= category.target ? '+' : ''}{category.percentage - category.target}%
                          </span>
                        </div>
                      </div>
                    ))}
                ...
                <div>
                  <h4 className="text-lg font-semibold text-white mb-4">Balance Metrics</h4>
                  <div className="space-y-4">
                    <div className="bg-slate-700 rounded-lg p-6 text-center">
                      <div className="text-4xl font-bold text-green-400 mb-2">94%</div>
                      <div className="text-lg font-medium text-white mb-1">Overall Balance Score</div>
                      <div className="text-sm text-gray-400">Excellent distribution</div>
                    </div>
            ...
            <CardContent>
              <div className="grid md:grid-cols-3 gap-6">
                <div className="bg-slate-700 rounded-lg p-6 text-center">
                  ...
                  <h4 className="text-lg font-semibold text-purple-400 mb-2">Training Dataset</h4>
                ...
                <div className="bg-slate-700 rounded-lg p-6 text-center">
                  <div className="text-4xl mb-4">📈</div>
                  <h4 className="text-lg font-semibold text-blue-400 mb-2">Quality Report</h4>
                  <p className="text-gray-300 text-sm mb-4">
                ...
                <div className="bg-slate-700 rounded-lg p-6 text-center">
                  ...
                  <h4 className="text-lg font-semibold text-green-400 mb-2">API Integration</h4>
                  <p className="text-gray-300 text-sm mb-4">
