...
import type { ClinicalCase, PatientInfo, PresentingProblemEvent } from '../../lib/types/psychology-pipeline';
...
const ScenarioGenerationDemo: React.FC = () => {
  const [profileData, setProfileData] = useState<ClientProfileFormData>({
    ...
    presentingProblemDevelopment: [
      { time: '6 months ago', description: 'Started new high-pressure job position' },
      { time: '3 months ago', description: 'Began experiencing sleep difficulties and worry' },
      { time: '1 month ago', description: 'Anxiety symptoms intensified, affecting work performance' }
    ],
    complexity: 'medium',
    therapeuticApproach: ['CBT', 'Mindfulness'],
    culturalFactors: ['Urban professional culture', 'Work-life balance expectations']
  });

  const [generatedProfile, setGeneratedProfile] = useState<Partial<ClinicalCase> | null>(null);
  ...
  const complexityOptions = [
    { value: 'low', label: 'Low - Single presenting issue, stable support system' },
    { value: 'medium', label: 'Medium - Multiple factors, some comorbidity' },
    { value: 'high', label: 'High - Complex trauma, multiple diagnoses, crisis risk' }
  ];
  const therapeuticApproaches = [
    'CBT', 'DBT', 'Psychodynamic', 'Humanistic', 'EMDR', 'Mindfulness', 'Solution-Focused', 'Family Systems'
  ];
  ...
  const updatePresentingProblemEvent = (index: number, field: 'time' | 'description', value: string) => {
    setProfileData(prev => ({
      ...prev,
      presentingProblemDevelopment: prev.presentingProblemDevelopment.map((event, i) =>
  ...
  const removePresentingProblemEvent = (index: number) => {
    setProfileData(prev => ({
      ...prev,
      presentingProblemDevelopment: prev.presentingProblemDevelopment.filter((_, i) => i !== index)
    }));
  };
  ...
  const generateProfile = async () => {
    ...
    try {
      ...
      const { generateClientScenario } = await import('../../lib/api/psychology-pipeline-demo');
      ...
      const generated: Partial<ClinicalCase> = {
        ...
        presentingProblemDevelopment: profileData.presentingProblemDevelopment,
        clinicalFormulation: {
          provisionalDiagnosis: ['Generalized Anxiety Disorder (300.02)', 'Adjustment Disorder with Anxiety (309.24)'],
          contributingFactors: {
            biological: ['Possible genetic predisposition to anxiety', 'Sleep disruption affecting stress response'],
            psychological: ['Perfectionist tendencies', 'Catastrophic thinking patterns', 'Low distress tolerance'],
            social: ['High-pressure work environment', 'Limited work-life boundaries', 'Social comparison pressures']
          },
        ...
        treatmentPlan: {
          ...
          outcomeMeasures: ['GAD-7', 'Beck Anxiety Inventory', 'Work and Social Adjustment Scale']
        }
      };
    ...
    <div className="scenario-generation-demo p-6 max-w-7xl mx-auto">
      <div className="mb-8">
        <div className="flex justify-between items-start">
          <div>
            <h2 className="text-3xl font-bold text-gray-900 mb-2">Scenario Generation Showcase</h2>
            <p className="text-gray-600">Create comprehensive client profiles for therapeutic training scenarios</p>
          </div>
          ...
          <div className="flex flex-col items-end gap-2">
            <div className={`flex items-center gap-2 px-3 py-1 rounded-full text-sm ${
              ...
              apiConnectionStatus === 'testing' ? 'bg-yellow-100 text-yellow-800' :
              'bg-red-100 text-red-800'
            }`}>
              <div className={`w-2 h-2 rounded-full ${
                apiConnectionStatus === 'connected' ? 'bg-green-500' :
                apiConnectionStatus === 'testing' ? 'bg-yellow-500 animate-pulse' :
                'bg-red-500'
              }`}></div>
              <span>
                {apiConnectionStatus === 'connected' ? 'API Connected' :
                 apiConnectionStatus === 'testing' ? 'Testing Connection' :
                 'API Disconnected'}
              </span>
            </div>
            ...
            {generationMetadata && (
              <div className="text-xs text-gray-500 text-right">
                <div>Quality: {generationMetadata.qualityScore.toFixed(1)}%</div>
                <div>Balance: {generationMetadata.balanceScore.toFixed(1)}%</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-lg shadow-md p-6">
            <h3 className="text-xl font-semibold mb-4 text-gray-800">Client Profile Creation</h3>
            ...
            <div className="space-y-4 mt-6">
              ...
              <div>
                <div className="flex justify-between items-center mb-2">
                  <label className="block text-sm font-medium text-gray-700">Problem Development Timeline</label>
                  <button
                ...
                <div className="space-y-2">
                  {profileData.presentingProblemDevelopment.map((event, index) => (
                    <div key={index} className="flex gap-2 items-start">
                      <input
                        type="text"
                        placeholder="Timeline (e.g., 3 months ago)"
                        value={event.time}
                        onChange={(e) => updatePresentingProblemEvent(index, 'time', e.target.value)}
                      ...
                      <input
                        ...
                        onChange={(e) => updatePresentingProblemEvent(index, 'description', e.target.value)}
                ...
                {complexityOptions.map(option => (
                  <label key={option.value} className="flex items-center">
                    <input
                      ...
                      onChange={(e) => handleComplexityChange(e.target.value as 'low' | 'medium' | 'high')}
                      className="mr-2"
                    />
            ...
            <div className="space-y-4 mt-6">
              <h4 className="font-medium text-gray-700">Preferred Therapeutic Approaches</h4>
              <div className="grid grid-cols-2 gap-2">
                {therapeuticApproaches.map(approach => (
                  <label key={approach} className="flex items-center">
                    <input
                      type="checkbox"
                      checked={profileData.therapeuticApproach.includes(approach)}
            ...
            >
              {isGenerating ? 'Generating Profile...' : 'Generate Comprehensive Profile'}
            </button>
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-gray-50 rounded-lg shadow-md p-6">
            <h3 className="text-xl font-semibold mb-4 text-gray-800">Generated Clinical Case</h3>
            
            {!generatedProfile ? (
              <div className="text-center py-8 text-gray-500">
                <p>Configure the client profile and click "Generate Comprehensive Profile" to see the AI-generated clinical case.</p>
              </div>
            ) : (
              <div className="space-y-6">
                {/* Case ID */}
                <div>
                  ...
                  <p className="text-sm bg-white p-2 rounded border">{generatedProfile.caseId}</p>
                </div>

                {/* Patient Summary */}
                <div>
                  <h4 className="font-medium text-gray-700 mb-2">Patient Summary</h4>
                  <div className="bg-white p-3 rounded border text-sm">
                    <p><strong>Age:</strong> {generatedProfile.patientInfo?.age}</p>
                    <p><strong>Gender:</strong> {generatedProfile.patientInfo?.gender}</p>
                    <p><strong>Occupation:</strong> {generatedProfile.patientInfo?.occupation}</p>
                    <p><strong>Background:</strong> {generatedProfile.patientInfo?.background}</p>
                  </div>
                </div>
                ...
                {generatedProfile.clinicalFormulation && (
                  <div>
                    <h4 className="font-medium text-gray-700 mb-2">Clinical Formulation</h4>
                    <div className="bg-white p-3 rounded border text-sm space-y-3">
                      <div>
                        <strong>Provisional Diagnoses:</strong>
                        <ul className="list-disc list-inside mt-1">
                          {generatedProfile.clinicalFormulation.provisionalDiagnosis.map((diagnosis, i) => (
                      ...
                      <div>
                        ...
                        <div className="mt-1 space-y-1">
                          <div><em>Biological:</em> {generatedProfile.clinicalFormulation.contributingFactors.biological.join(', ')}</div>
                          <div><em>Psychological:</em> {generatedProfile.clinicalFormulation.contributingFactors.psychological.join(', ')}</div>
                          <div><em>Social:</em> {generatedProfile.clinicalFormulation.contributingFactors.social.join(', ')}</div>
                        </div>
                      </div>
                      
                      <div>
                        <strong>Summary:</strong>
                        <p className="mt-1">{generatedProfile.clinicalFormulation.summary}</p>
                      </div>
                    </div>
                  </div>
                )}
                  ...
                  <div>
                    ...
                    <div className="bg-white p-3 rounded border text-sm space-y-3">
                      <div>
                        <strong>Goals:</strong>
                        <div className="mt-1">
                          <div><em>Short-term:</em></div>
                          <ul className="list-disc list-inside ml-4">
                            {generatedProfile.treatmentPlan.goals.shortTerm.map((goal, i) => (
                              <li key={i}>{goal}</li>
                            ))}
                          </ul>
                          <div className="mt-2"><em>Long-term:</em></div>
                          <ul className="list-disc list-inside ml-4">
                            {generatedProfile.treatmentPlan.goals.longTerm.map((goal, i) => (
                              <li key={i}>{goal}</li>
                            ))}
                          </ul>
                        </div>
                      ...
                      <div>
                        ...
                        <ul className="list-disc list-inside mt-1">
                          {generatedProfile.treatmentPlan.interventions.map((intervention, i) => (
                      ...
                      <div>
                        <strong>Modalities:</strong> {generatedProfile.treatmentPlan.modalities.join(', ')}
                      </div>
                      
                      <div>
                        <strong>Outcome Measures:</strong> {generatedProfile.treatmentPlan.outcomeMeasures.join(', ')}
                      </div>
                    </div>
                  </div>
                )}
