...
class ClinicalAccuracyValidator:
    ...
    async def assess_clinical_accuracy(
        ...
        therapeutic_score = await self._assess_therapeutic_appropriateness(response, clinical_context)
        ...
        expert_validation_needed = (
            confidence_score < self.config["expert_validation_threshold"] or
            safety_assessment.overall_risk.value in ["high", "critical"]
        )
    ...
    async def _assess_dsm5_compliance(
        ...
        mdd_indicators = [
            ...
            "sleep", "appetite", "energy", "concentration", "guilt"
        ]
        mdd_score = sum(1 for indicator in mdd_indicators if indicator in response_lower)
        ...
        anxiety_indicators = [
            ...
            "restless", "tense", "irritable"
        ]
        anxiety_score = sum(1 for indicator in anxiety_indicators if indicator in response_lower)
    ...
    async def _assess_pdm2_compliance(
        ...
        if any(word in response_lower for word in ["grandiose", "special", "entitled", "admiration"]):
            assessment.personality_patterns.append("narcissistic")
        
        # Check for depressive patterns
        if any(word in response_lower for word in ["worthless", "guilty", "self-critical", "inadequate"]):
    ...
    async def _assess_therapeutic_appropriateness(
        ...
        intervention_matches = sum(
            1 for intervention in appropriate_interventions 
            if any(word in response_lower for word in intervention.split('_'))
        )
        
        score.intervention_appropriateness = min(1.0, intervention_matches / max(1, len(appropriate_interventions)))
        ...
        ethical_score = sum(1 for indicator in ethical_indicators if indicator in response_lower)
        ...
        violation_score = sum(1 for violation in boundary_violations if violation in response_lower)
    ...
    async def _assess_safety_compliance(
        ...
        suicide_score = sum(1 for indicator in suicide_indicators if indicator in response_lower)
        ...
        self_harm_score = sum(1 for indicator in self_harm_indicators if indicator in response_lower)
        ...
        violence_score = sum(1 for indicator in violence_indicators if indicator in response_lower)
        ...
        max_risk = max(risk_levels, key=lambda x: ["minimal", "low", "moderate", "high", "critical"].index(x.value))
    ...
    def _calculate_overall_accuracy(
        ...
        pdm2_score = np.mean(list(pdm2.mental_functioning.values())) if pdm2.mental_functioning else 0.5
    ...
    def _generate_warnings(
        self,
        safety: SafetyAssessment,
        accuracy: ClinicalAccuracyLevel
    ) -> List[str]:
