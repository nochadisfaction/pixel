...
class TestExpertValidationInterface:
    ...
    def sample_assessment_result(self):
        ...
        return ClinicalAccuracyResult(
            ...
            safety_assessment=SafetyAssessment(
                overall_risk=SafetyRiskLevel.LOW
            ),
    ...
    def test_list_experts_by_specialty(self, interface):
        ...
        experts = interface.list_experts(
            specialty=ExpertSpecialty.CLINICAL_PSYCHOLOGY
        )
        assert len(experts) > 0
        assert all(
            ExpertSpecialty.CLINICAL_PSYCHOLOGY in expert.specialties
    ...
    @pytest.mark.asyncio
    async def test_create_validation_request_success(
        ...
        assert (ExpertSpecialty.CLINICAL_PSYCHOLOGY in
    ...
    async def test_create_validation_request_auto_specialty(
        ...
        sample_assessment_result.safety_assessment.overall_risk = (
        ...
        request = interface.get_validation_request(request_id)
        assert (ExpertSpecialty.CRISIS_INTERVENTION in
                request.required_specialties)
    ...
    def test_determine_required_specialties_substance_abuse(
        self, interface
    ):
    ...
    def test_calculate_expert_suitability_score(
        self, interface, sample_expert
    ):
        ...
        score = interface._calculate_expert_suitability_score(
            sample_expert, request
        )
    ...
    def test_assign_expert_to_request(self, interface, sample_expert):
        ...
        result = interface._assign_expert_to_request(
            sample_expert.expert_id, "test_request"
        )
    ...
    def test_assign_expert_to_request_duplicate(
        self, interface, sample_expert
    ):
        ...
        # First assignment
        result1 = interface._assign_expert_to_request(
            sample_expert.expert_id, "test_request"
        )
        # Second assignment (duplicate)
        result2 = interface._assign_expert_to_request(
            sample_expert.expert_id, "test_request"
        )
