...
from .clinical_accuracy_validator import (
    ClinicalAccuracyResult, ClinicalContext, SafetyRiskLevel
)
...
class SafetyEthicsValidator:
        ...
        indicators = [
            ...
            SafetyIndicator(
                ...
                keywords=[
                    ...
                    "burning", "hitting myself"
                ],
                patterns=[
                    r"(cut|hurt|harm) myself",
                    r"self.harm",
                    r"(burning|hitting) myself"
                ],
                immediate_action_required=True
            ),
            SafetyIndicator(
            ...
            SafetyIndicator(
                ...
                keywords=[
                    "child abuse", "hit my child", "neglect", "inappropriate touch",
                    "sexual abuse"
                ],
                patterns=[
    ...
    async def validate_compliance(
        ...
        safety_result = await self._assess_safety_compliance(
            response_text, clinical_context
        )
        ethics_result = await self._assess_ethics_compliance(
            response_text, clinical_context
        )
        legal_result = await self._assess_legal_compliance(
            response_text, clinical_context
        )
        
        # Collect all violations
        violations = []
        violations.extend(await self._detect_safety_violations(
            response_text, clinical_context, safety_result
        ))
        violations.extend(await self._detect_ethics_violations(
            response_text, clinical_context, ethics_result
        ))
        violations.extend(await self._detect_legal_violations(
            response_text, clinical_context, legal_result
        ))
    ...
    async def _assess_safety_compliance(
        ...
        if not category_risks:
            ...
            overall_risk = next(
                risk for risk, value in risk_values.items() 
    ...
    async def _assess_ethics_compliance(
        ...
        for guideline in self.ethics_guidelines.values():
            ...
            if compliance_level in [ComplianceLevel.FULLY_COMPLIANT, ComplianceLevel.MOSTLY_COMPLIANT]:
        ...
        if not category_compliance:
            ...
            overall_compliance = next(
                comp for comp, value in compliance_values.items()
                if value <= avg_compliance
            )
    ...
    async def _detect_legal_violations(
        ...
        for requirement_id in legal_result.requirements_violated:
            ...
            violation = ComplianceViolation(
                ...
                severity=ViolationSeverity.SEVERE if requirement.mandatory else ViolationSeverity.MAJOR,
    ...
    def _generate_recommendations(self, violations: List[ComplianceViolation]) -> List[str]:
        ...
        if violations:
            recommendations.extend([
                "Review safety and ethics protocols",
