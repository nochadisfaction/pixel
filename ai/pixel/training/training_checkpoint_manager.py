...
class CheckpointManager:
    ...
    def _save_checkpoint_sync(
        ...
        try:
            with self.save_lock:
                ...
                if self.config.atomic_saves:
                    ...
                    if self.config.validate_on_save:
                        ...
                        if not validation_results["is_valid"]:
                            ...
                            raise RuntimeError(f"Checkpoint validation failed: {validation_results['errors']}")
                ...
                self.logger.info(f"Checkpoint saved successfully: {checkpoint_id} ({metadata.file_size_mb:.1f} MB)")
    ...
    def load_checkpoint(self, checkpoint_id: Optional[str] = None) -> Optional[TrainingState]:
        ...
        try:
            ...
            self.logger.info(f"Resuming from step {training_state.step}, epoch {training_state.epoch}")
    ...
    def get_best_checkpoint_id(self, metric_name: Optional[str] = None) -> Optional[str]:
        ...
        if mode == "min":
            best_checkpoint = min(best_checkpoints, key=lambda c: getattr(c, metric_name, float('inf')))
        else:
            best_checkpoint = max(best_checkpoints, key=lambda c: getattr(c, metric_name, float('-inf')))
    ...
    def _cleanup_old_checkpoints(self) -> None:
        ...
        try:
            ...
            sorted_checkpoints = sorted(self.checkpoint_history, key=lambda c: c.timestamp, reverse=True)
            ...
            if self.config.keep_last_n_checkpoints > 0:
                keep_checkpoints.update(
                    c.checkpoint_id for c in sorted_checkpoints[:self.config.keep_last_n_checkpoints]
                )
    ...
    def _save_checkpoint_history(self) -> None:
        ...
        try:
            ...
            for checkpoint_data in history_data:
                if "timestamp" in checkpoint_data and isinstance(checkpoint_data["timestamp"], datetime):
    ...
    def _load_checkpoint_history(self) -> None:
        ...
        try:
            ...
            if history_path.exists():
                ...
                for checkpoint_data in history_data:
                    if "timestamp" in checkpoint_data and isinstance(checkpoint_data["timestamp"], str):
...
def create_training_state_from_model(
    ...
    **kwargs
    ...
    return TrainingState(
        ...
        numpy_rng_state={"bit_generator": np.random.get_state()[0], "state": np.random.get_state()},
