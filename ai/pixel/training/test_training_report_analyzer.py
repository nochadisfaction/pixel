...
from training_report_analyzer import (
    ReportConfig, TrainingMetrics, ValidationResult, AnomalyReport, CheckpointInfo,
    TrainingReport, DatabaseConnector, MetricsAnalyzer, TrainingReportAnalyzer
)
...
class TestMetricsAnalyzer(unittest.TestCase):
    ...
    def test_detect_plateaus(self):
        ...
        plateau_data = pd.DataFrame({
            'step': range(200),
            'loss': [1.0 - 0.01 * i if i < 50 else 0.5 + 0.001 * (i - 50) if i < 150 else 0.6 - 0.01 * (i - 150) for i in range(200)]
        })
    ...
    def _create_mock_databases(self):
        ...
        with sqlite3.connect(self.config.metrics_db_path) as conn:
            ...
            for i in range(10):
                ...
                conn.execute("""
                    ...
                    (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    i * 100, i, timestamp, 1.0 - 0.1 * i, 0.5 - 0.05 * i,
                    0.3 - 0.03 * i, 0.2 - 0.02 * i, 0.1 + 0.08 * i,
                    0.2 + 0.06 * i, 0.3 + 0.05 * i, 1e-4, 0.8, 1000.0, 0.5
                ))
            conn.commit()
        ...
        with sqlite3.connect(self.config.validation_db_path) as conn:
            ...
            for i in range(5):
                ...
                conn.execute("""
                    ...
                    (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    i * 200, timestamp, 0.8 - 0.1 * i, 0.2 + 0.1 * i,
                    0.3 + 0.1 * i, 0.4 + 0.08 * i, 10 - i, 0.2 + 0.1 * i, True
                ))
            conn.commit()
        ...
        with sqlite3.connect(self.config.anomaly_db_path) as conn:
            ...
            conn.execute("""
                ...
                (?, ?, ?, ?, ?, ?, ?)
            """, (
                anomaly_time, "loss_spike", "medium", "Loss increased unexpectedly",
                '["total_loss"]', '["Check learning rate"]', True
            ))
            conn.commit()
