...
class MetricsAnalyzer:
    ...
    @staticmethod
    def calculate_progression(data: pd.DataFrame, metric_col: str, 
    ...
    @staticmethod
    def detect_plateaus(data: pd.DataFrame, metric_col: str, 
...
class VisualizationEngine:
    ...
    def create_metrics_overview(self, metrics_df: pd.DataFrame) -> go.Figure:
        ...
        fig = make_subplots(
            rows=2, cols=2,
            subplot_titles=['Loss Progression', 'EQ Score Progression', 
                          'Clinical Accuracy', 'Persona Consistency'],
            vertical_spacing=0.08
        )
        
        # Loss progression
        fig.add_trace(
            go.Scatter(x=metrics_df['step'], y=metrics_df['total_loss'],
                      name='Total Loss', line=dict(color=self.colors['primary'])),
            row=1, col=1
        )
        fig.add_trace(
            go.Scatter(x=metrics_df['step'], y=metrics_df['eq_loss'],
                      name='EQ Loss', line=dict(color=self.colors['secondary'])),
            row=1, col=1
        )
        
        # EQ Score progression
        fig.add_trace(
            go.Scatter(x=metrics_df['step'], y=metrics_df['eq_score'],
                      name='EQ Score', line=dict(color=self.colors['success'])),
            row=1, col=2
        )
        
        # Clinical accuracy
        fig.add_trace(
            go.Scatter(x=metrics_df['step'], y=metrics_df['clinical_accuracy'],
                      name='Clinical Accuracy', line=dict(color=self.colors['info'])),
            row=2, col=1
        )
        
        # Persona consistency
        fig.add_trace(
            go.Scatter(x=metrics_df['step'], y=metrics_df['persona_consistency'],
                      name='Persona Consistency', line=dict(color=self.colors['warning'])),
            row=2, col=2
        )
    ...
    def create_checkpoint_analysis(self, checkpoints: List[CheckpointInfo]) -> go.Figure:
        ...
        if not checkpoints:
            fig = go.Figure()
            fig.add_annotation(text="No checkpoint data available", 
                             xref="paper", yref="paper",
                             x=0.5, y=0.5, showarrow=False)
            return fig
...
class TrainingReportAnalyzer:
    ...
    def generate_recommendations(self, metrics_df: pd.DataFrame, 
                               anomalies: List[AnomalyReport]) -> Tuple[List[str], List[str]]:
        ...
        if not metrics_df.empty:
            ...
            if loss_reduction < 0.1:
                recommendations.append("Loss reduction is minimal. Consider adjusting learning rate or architecture.")
            
            # Analyze EQ progression
            eq_progress = metrics_df['eq_score'].iloc[-1] - metrics_df['eq_score'].iloc[0]
            if eq_progress < 0.05:
                recommendations.append("EQ score improvement is slow. Review EQ training data quality.")
            ...
            if avg_grad_norm > 1.0:
                ...
                recommendations.append("Low gradient norms detected. Consider increasing learning rate.")
    ...
    def generate_report(self) -> TrainingReport:
        ...
        loss_progression = analyzer.calculate_progression(metrics_df, 'total_loss', self.config.trend_window)
        eq_progression = analyzer.calculate_progression(metrics_df, 'eq_score', self.config.trend_window)
        clinical_progression = analyzer.calculate_progression(metrics_df, 'clinical_accuracy', self.config.trend_window)
        persona_progression = analyzer.calculate_progression(metrics_df, 'persona_consistency', self.config.trend_window)
        ...
        checkpoint_summary = {
            ...
            'avg_size_mb': sum(c.file_size_mb for c in checkpoints) / len(checkpoints) if checkpoints else 0.0
        }
    ...
    def _generate_pdf_report(self, report: TrainingReport, output_path: Path):
        ...
        with PdfPages(str(output_path)) as pdf:
            ...
            if report.training_efficiency:
                ...
                axes[1, 0].text(0.5, 0.5, 'No efficiency data', 
...
def main():
    ...
    config = ReportConfig(
        report_name="test_training_report",
        last_n_days=7,
