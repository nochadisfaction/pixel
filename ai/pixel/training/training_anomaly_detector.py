...
class TrainingAnomalyDetector:
    ...
    def _detect_loss_anomalies(self, metrics: TrainingMetrics) -> List[AnomalyAlert]:
        ...
        is_anomaly, z_score = self.statistical_detector.detect_anomaly('total_loss', metrics.total_loss)
        ...
        if len(self.metrics_history) >= 50:
            ...
            if loss_variance < 1e-6:  # Very small variance indicates plateau
                alerts.append(AnomalyAlert(
                    ...
                    metrics={'loss_variance': loss_variance, 'recent_losses': recent_losses[-10:]},
                    suggested_actions=[
            ...
            if domain in self.last_eq_scores:
                ...
                if score_drop > self.config.eq_score_degradation_threshold:
                    alerts.append(AnomalyAlert(
                        ...
                        metrics={'domain': domain, 'current_score': score, 'previous_score': self.last_eq_scores[domain]},
                        suggested_actions=[
            ...
            if category in self.last_clinical_accuracy:
                ...
                if accuracy_drop > self.config.clinical_accuracy_drop_threshold:
                    alerts.append(AnomalyAlert(
                        ...
                        metrics={'category': category, 'current_accuracy': accuracy, 'previous_accuracy': self.last_clinical_accuracy[category]},
                        suggested_actions=[
        ...
        if len(self.metrics_history) >= 100:
            ...
            if len(set(f"{loss:.4f}" for loss in recent_losses)) < 5:  # Very little variation
                alerts.append(AnomalyAlert(
                    ...
                    metrics={'recent_loss_variation': len(set(f"{loss:.4f}" for loss in recent_losses))},
                    suggested_actions=[
        ...
        if len(self.metrics_history) >= 20:
            ...
            if memory_trend > 50:  # Memory increasing by >50MB per step on average
                alerts.append(AnomalyAlert(
                    ...
                    metrics={'memory_trend': memory_trend, 'current_memory': metrics.memory_usage_mb},
                    suggested_actions=[
