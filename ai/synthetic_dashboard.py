...
with tabs[0]:
    ...
    st.markdown(f"<div class='metric-anim'>Total Q&A Pairs: {len(df):,}</div>", unsafe_allow_html=True)
...
with tabs[1]:
    ...
    st.markdown("<small style='color:#7df9ff;'>Tip: Use the search box to filter Q&A pairs.</small>", unsafe_allow_html=True)
...
with tabs[2]:
    ...
    with col1:
        ...
        if 'prompt' in df.columns and 'response' in df.columns:
            ...
            chart = alt.Chart(length_df.reset_index()).transform_fold(
                ...
                x=alt.X('Length:Q', bin=alt.Bin(maxbins=40), axis=alt.Axis(labelColor='#7df9ff', titleColor='#7df9ff')),
                y=alt.Y('count()', stack=None, axis=alt.Axis(labelColor='#7df9ff', titleColor='#7df9ff')),
    ...
    with col2:
        ...
        if 'prompt' in df.columns:
            ...
            wc = WordCloud(width=400, height=200, background_color='#181825', mode='RGBA', colormap='winter').generate(text)
            buf = BytesIO()
            wc.to_image().save(buf, format='PNG')
            img_b64 = base64.b64encode(buf.getvalue()).decode()
            st.markdown(f"<img src='data:image/png;base64,{img_b64}' style='width:100%; border-radius:12px; box-shadow:0 2px 12px #00fff733;' />", unsafe_allow_html=True)
    st.markdown("---")
