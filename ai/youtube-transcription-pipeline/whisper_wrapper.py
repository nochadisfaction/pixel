...
def main():
    ...
    print(
        f"[DEBUG] Faster-whisper wrapper starting at {time.strftime('%H:%M:%S')}",
        file=sys.stderr,
    )
    ...
    try:
        ...
        print(
            "[DEBUG] faster-whisper not found, attempting installation...",
            file=sys.stderr,
        )
        try:
            ...
            if not sys.executable or not os.path.isabs(sys.executable):
                print(
                    "[ERROR] Untrusted or invalid Python executable path.",
                    file=sys.stderr,
                )
                sys.exit(1)
            ...
            print(
                "[ERROR] Could not import faster-whisper after installation.",
                file=sys.stderr,
            )
            sys.exit(1)
    try:
        ...
        print(
            f"[DEBUG] Transcription completed in {transcribe_time:.2f}s ({segment_count} segments)",
            file=sys.stderr,
        )
        ...
        print(
            f"[DEBUG] Transcription length: {len(transcription)} characters",
            file=sys.stderr,
        )
