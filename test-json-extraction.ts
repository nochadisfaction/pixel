...
import type {
  RoutingInput,
  LLMResponse,
} from './src/lib/ai/mental-llama/types/mentalLLaMATypes'
...
async function testRouterJsonParsing() {
  ...
  const testCases = [
    {
      ...
      llmResponse:
        'Here is the result: {"category": "depression", "confidence": 0.8, "reasoning": "User mentions feeling down", "is_critical_intent": false}',
    ...
    },
    {
      ...
      llmResponse:
        'Analysis: {"category": "anxiety", "confidence": 0.9, "reasoning": "Shows signs of worry", "is_critical_intent": false} End of response.',
    ...
    },
    {
      ...
      llmResponse:
        'Result: {"category": "stress", "confidence": 0.7, "reasoning": "Patient says \\"I feel overwhelmed\\" frequently", "is_critical_intent": false}',
    ...
    },
    {
      ...
      llmResponse:
        '{"category": "crisis_severe", "confidence": 0.95, "reasoning": "Direct suicidal ideation expressed", "is_critical_intent": true}',
    ...
    },
    {
      ...
      llmResponse:
        '{"category": "depression", "confidence": 0.6, "reasoning": "First response", "is_critical_intent": false} and then {"category": "anxiety", "confidence": 0.8, "reasoning": "Second response", "is_critical_intent": false}',
      expectedCategory: 'depression',
  ...
  const processTestCase = async (testCase: (typeof testCases)[0]) => {
    ...
    try {
      ...
      console.log(
        `✅ Decision received: ${JSON.stringify(
          {
      ...
      if (testCase.shouldSucceed) {
        const passedCategoryCheck = testCase.expectedCategory
          ? decision.targetAnalyzer.includes(
              testCase.expectedCategory.split('_')[0],
            )
          : true
        const passedConfidenceCheck = testCase.expectedConfidence
          ? Math.abs(decision.confidence - testCase.expectedConfidence) < 0.1
          : true

        console.log(
          `✅ Category check: ${passedCategoryCheck ? 'PASS' : 'FAIL'}`,
        )
        console.log(
          `✅ Confidence check: ${passedConfidenceCheck ? 'PASS' : 'FAIL'}`,
        )
        console.log(
          `✅ JSON parsed successfully: ${decision.method === 'llm' ? 'YES' : 'NO'}`,
        )
      } else {
        // For failure cases, expect fallback behavior
        const usedFallback =
          decision.method.includes('fallback') ||
          decision.method === 'keyword' ||
          decision.method === 'default'
        console.log(
          `✅ Used fallback as expected: ${usedFallback ? 'YES' : 'NO'}`,
        )
      }
    } catch (error) {
      console.log(
