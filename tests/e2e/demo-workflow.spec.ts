...
test.describe('Psychology Pipeline Demo - Complete Workflow', () => {
  ...
  test('Complete pipeline workflow from ingestion to export', async ({ page }) => {
    ...
    await test.step('Data Ingestion', async () => {
      ...
      const testFilePath = path.join(__dirname, '../fixtures/test-psychology-data.json');
    ...
    await test.step('Content Validation', async () => {
      ...
      await textArea.fill('The client presents with symptoms of generalized anxiety disorder requiring cognitive behavioral therapy intervention.');
    ...
    await test.step('Category Balancing', async () => {
      ...
      const balanceScore = await page.locator('[data-testid="balance-score"]').textContent();
    ...
    await test.step('Results Export', async () => {
      ...
      await expect(page.locator('text=Executive Summary Report')).toBeVisible({ timeout: 10000 });
    });

    // Step 5: API Integration Testing
    await test.step('API Integration', async () => {
      // Test API connections
      const testConnectionButtons = page.locator('button:has-text("Test Connection")');
      ...
      const sendDataButton = page.locator('button:has-text("Send Data")').first();
      if (await sendDataButton.isVisible()) {
        await sendDataButton.click();
        await expect(page.locator('text=Data sent successfully')).toBeVisible({ timeout: 10000 });
      }
    });
  });
  ...
  test('Error handling throughout the pipeline', async ({ page }) => {
    ...
    await test.step('Validation Error Handling', async () => {
      ...
      const textArea = page.locator('[placeholder*="Enter psychology content"]');
      await textArea.fill('This content contains inappropriate language and unprofessional terminology that should trigger validation errors.');
  ...
  test('Real-time features and responsiveness', async ({ page }) => {
    ...
    await test.step('Real-time Balancing', async () => {
      ...
      const initialRatio = await page.locator('[data-testid="anxiety-ratio"]').textContent();
      ...
      const updatedRatio = await page.locator('[data-testid="anxiety-ratio"]').textContent();
