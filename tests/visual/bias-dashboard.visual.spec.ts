...
const mockDashboardData = {
  ...
  trends: [
    { timestamp: '2024-01-15T09:00:00Z', biasScore: 0.15, sessionCount: 45, alertCount: 1 },
    { timestamp: '2024-01-15T10:00:00Z', biasScore: 0.23, sessionCount: 52, alertCount: 2 },
    { timestamp: '2024-01-15T11:00:00Z', biasScore: 0.18, sessionCount: 48, alertCount: 1 },
  ],
  demographics: {
  ...
  alerts: [
    {
      ...
      level: 'critical',
      type: 'Systemic Bias',
      message: 'Critical systemic bias detected across multiple demographic groups',
...
class DashboardVisualTestUtils {
  ...
  static async waitForDashboardLoad(page: Page) {
    ...
    await page.waitForSelector('[data-testid="bias-dashboard"]', { timeout: 10000 });
...
test.describe('Bias Dashboard - Visual Regression Tests', () => {
  ...
  test.describe('Desktop Layout Visual Tests', () => {
    test.beforeEach(async ({ page }) => {
      await page.setViewportSize(DashboardVisualTestUtils.getViewportSizes().desktop);
    });

    test('should match baseline dashboard in normal state', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="bias-dashboard"]')).toHaveScreenshot(
        'dashboard-normal-desktop.png',
    ...
    test('should match dashboard with critical alerts', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="bias-dashboard"]')).toHaveScreenshot(
        'dashboard-critical-alerts-desktop.png',
        { fullPage: true, threshold: 0.2 }
      );
    });
    ...
    test('should match demographics tab content', async ({ page }) => {
      ...
      if (await demographicsTab.isVisible()) {
        ...
        const demographicsContent = page.locator('[data-testid="demographics-content"]');
  ...
  test.describe('Mobile Layout Visual Tests', () => {
    test.beforeEach(async ({ page }) => {
      await page.setViewportSize(DashboardVisualTestUtils.getViewportSizes().mobile);
    });
  ...
  test.describe('Tablet Layout Visual Tests', () => {
    test.beforeEach(async ({ page }) => {
      await page.setViewportSize(DashboardVisualTestUtils.getViewportSizes().tablet);
    });
  ...
  test.describe('Chart Visual Consistency Tests', () => {
    ...
    test('should match bias trend chart visualization', async ({ page }) => {
      ...
      if (await biasChart.isVisible()) {
        await expect(biasChart).toHaveScreenshot(
          'bias-trend-chart.png',
          { threshold: 0.2 }
        );
      }
    });
    ...
    test('should match demographic distribution charts', async ({ page }) => {
      ...
      if (await demographicsTab.isVisible()) {
        ...
        const demographicCharts = page.locator('[data-testid="demographic-charts"]');
    ...
    test('should match alert visualization with different severity levels', async ({ page }) => {
      await DashboardVisualTestUtils.setupMockData(page, criticalAlertData);
      await page.goto('/admin/bias-detection');
      await DashboardVisualTestUtils.waitForDashboardLoad(page);
