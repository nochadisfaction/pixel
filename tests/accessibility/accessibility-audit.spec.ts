...
test.describe('Accessibility Audit and Compliance', () => {
  ...
  test('Keyboard navigation and focus management', async () => {
    await test.step('Tab navigation through all interactive elements', async () => {
      ...
      for (let i = 0; i < 50; i++) { // Limit to prevent infinite loop
        ...
        const isSameElement = await currentElement.evaluate((el, other) => el === other, await nextElement.elementHandle());
  ...
  test('Color contrast and visual accessibility', async () => {
    await test.step('Color contrast ratios', async () => {
      ...
      const textElements = page.locator('p, span, div, button, a').filter({ hasText: /\w+/ });
      const textCount = await textElements.count();
      
      const contrastIssues = [];
      
      for (let i = 0; i < Math.min(textCount, 20); i++) { // Test first 20 elements
    ...
    await test.step('Focus indicators visibility', async () => {
      const focusableElements = page.locator('button, input, textarea, [role="button"], [role="tab"]');
  ...
  test('Motor accessibility and interaction', async () => {
    await test.step('Touch target sizes', async () => {
      const interactiveElements = page.locator('button, input, [role="button"], [role="tab"], a');
  ...
  test('Cognitive accessibility', async () => {
    ...
    await test.step('Help text and instructions', async () => {
      ...
      const helpElements = page.locator('[role="tooltip"], .help-text, [aria-describedby]');
