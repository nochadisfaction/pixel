  ...
  test.describe(`Mobile Responsiveness - ${device.name}`, () => {
    ...
    test('Touch interactions work properly', async ({ page }) => {
      await test.step('Touch targets meet minimum size requirements', async () => {
        const touchTargets = page.locator('button, input, [role="button"], [role="tab"], a');
      ...
      await test.step('Touch gestures and interactions', async () => {
        ...
        // Test touch typing
        await page.keyboard.type('Mobile touch input test for psychology content validation');
        ...
        if (cardCount > 0) {
          for (let i = 0; i < Math.min(cardCount, 3); i++) {
            ...
            if (cardBox) {
              ...
              expect(cardBox.width).toBeLessThanOrEqual((page.viewportSize()?.width || 0) - 20);
            }
          }
        }
      });
    ...
    test('Mobile performance and loading', async ({ page }) => {
      ...
      await test.step('Mobile memory usage', async () => {
        ...
        if (initialMemory > 0 && finalMemory > 0) {
          ...
          console.log(`${device.name} memory increase: ${memoryIncrease / 1024 / 1024}MB`);
    ...
    test('Mobile accessibility features', async ({ page }) => {
      ...
      await test.step('Mobile color contrast', async () => {
        ...
        const textElements = page.locator('p, span, div, button').filter({ hasText: /\w+/ });
  ...
  test.describe(`Tablet Responsiveness - ${device.name}`, () => {
    ...
    test('Tablet layout optimization', async ({ page }) => {
      ...
      await test.step('Tablet touch interactions', async () => {
        ...
        const textArea = page.locator('[placeholder*="Enter psychology content"]');
        await textArea.tap();
        await page.keyboard.type('Tablet input test with comprehensive psychology content for validation testing');
...
test.describe('Cross-Device Mobile Compatibility', () => {
  test('Consistent behavior across mobile devices', async ({ browser }) => {
    ...
    for (const device of mobileDevices.slice(0, 3)) { // Test first 3 devices
  ...
  test('Progressive enhancement across device capabilities', async ({ browser }) => {
    ...
    const lowEndDevice = {
      ...
      isMobile: true,
      hasTouch: true,
      userAgent: 'Mozilla/5.0 (iPhone; CPU iPhone OS 10_0 like Mac OS X) AppleWebKit/602.1.38'
    };
...
test.describe('Mobile Edge Cases and Error Handling', () => {
  ...
  test('Mobile input method compatibility', async ({ page }) => {
    ...
    await page.keyboard.type('Voice input test for mobile psychology validation');
