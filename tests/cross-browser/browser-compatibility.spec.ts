  ...
  test.describe(`Cross-Browser Compatibility - ${browserName}`, () => {
    ...
    test('Basic page functionality works across browsers', async () => {
      await test.step('Page loads and renders correctly', async () => {
        ...
        await expect(page.locator('[data-testid="data-ingestion-tab"]')).toBeVisible();
        await expect(page.locator('[data-testid="validation-tab"]')).toBeVisible();
        await expect(page.locator('[data-testid="category-balancing-tab"]')).toBeVisible();
      ...
      await test.step('Navigation between tabs works', async () => {
        ...
        await expect(page.locator('[data-testid="category-balancing-section"]')).toBeVisible();
        ...
        await expect(page.locator('[data-testid="data-ingestion-section"]')).toBeVisible();
      });
    ...
    test('File upload functionality across browsers', async () => {
      await test.step('File input works correctly', async () => {
        ...
        await expect(page.locator('text=test-file.json')).toBeVisible({ timeout: 10000 });
      });
    ...
    test('JavaScript functionality across browsers', async () => {
      await test.step('Event handling works correctly', async () => {
        ...
        // Test button clicks
        const simulateButton = page.locator('button:has-text("Simulate Influx")');
...
test.describe('Browser-Device Combinations', () => {
  ...
  deviceBrowserCombinations.forEach(({ device, browser }) => {
    test(`${device.name} with ${browser}`, async ({ playwright }) => {
      const browserInstance = await playwright[browser as keyof typeof playwright].launch();
