...
def create_test_audio(
    ...
    if clipping:
        ...
        max_val = max(samples)
        audio = audio._spawn(bytes((max_val if i % 2 == 0 else 0 for i in range(len(samples)))))  # noqa: SLF001  # type: ignore[attr-defined]
    return audio
...
def test_assess_audio_quality_fail_clipping(tmp_path: Path):  # type: ignore[unused-argument]
    ...
    audio.export(file_path, format="wav")
    result = vqa.assess_audio_quality(str(file_path))
    assert result["passed"] is False or result["clipping_ratio"] > vqa.DEFAULT_CONFIG["max_clipping_ratio"]
